...
| 항목 | 확인 포인트 |
|---|---|
| 먼저 볼 구간 | `통신 명세 표(공식 표준 양식)` |
| 다음 확인 구간 | `Comm SoT 요약` + `통신 상세 추적 표` |
| 제출 관점 요약 | 통신 계약(Comm)과 Flow/Req 연계 |

## 중간보고 핵심

- Comm는 메시지 계약과 SoT 정책(00f/ETH 계약)의 합치 여부를 우선 확인한다.
- 검토 순서는 `CAN ID 배정 정책 -> 통신 명세 표 -> 통신 상세 추적 표`로 고정한다.
- SIL stub alias는 문서와 구현 간 명칭 차이를 설명하는 보조 정보로 사용한다.
