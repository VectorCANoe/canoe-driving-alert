...
| 항목 | 확인 포인트 |
|---|---|
| 먼저 볼 구간 | `2. 기준 아키텍처(고정)` |
| 다음 확인 구간 | `3. 그림 구성(확정)` |
| 제출 관점 요약 | 아키텍처 그림 + 네트워크 흐름 그림의 정합성 |

## 중간보고 핵심

| 구분 | 핵심 메시지 |
|---|---|
| 채택 구조 | Option 1(`ETH_SW + Domain Gateway + Domain CAN + 중앙 경고코어`) |
| 검증 범위 | CANoe SIL, CAN + Ethernet(UDP) |
| 판정 규칙 | `Emergency > Zone`, `Ambulance > Police`, 동률 시 `ETA -> SourceID`, `Timeout Clear 1000ms` |
