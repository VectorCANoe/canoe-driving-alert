...
| 항목 | 확인 포인트 |
|---|---|
| 먼저 볼 구간 | `기능 정의 표(공식 표준 양식)` |
| 다음 확인 구간 | 하단 대표 매핑/정책 참조 섹션 |
| 제출 관점 요약 | 기능(How)과 요구(What)의 연결성 |

## 중간보고 핵심

- 01의 요구사항을 기능 단위로 분해한 기준 문서이며, 기능 책임은 도메인 중심으로 확인한다.
- 검토 순서는 `기능 정의 표 -> 대표 매핑 섹션`으로 고정한다.
- 전수 추적은 원문 SoT를 유지하고 제출본은 대표 연결만 보여준다.
