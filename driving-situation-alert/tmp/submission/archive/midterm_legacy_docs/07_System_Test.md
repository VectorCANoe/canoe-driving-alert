...
| 항목 | 확인 포인트 |
|---|---|
| 먼저 볼 구간 | `시스템 테스트 시나리오(공식 표준 양식)` |
| 다음 확인 구간 | 하단 ST 대표 추적/상태 구간 |
| 제출 관점 요약 | 운전자 관점 E2E 시나리오 검증 범위 |

## 중간보고 핵심

- ST는 운전자 관점 E2E 동작과 정책 일관성 확인에 집중한다.
- 검토 순서는 `시스템 테스트 시나리오 표 -> ST 대표 추적/상태`로 고정한다.
- Pass/Fail 공란은 실행증빙 수집 단계로 해석하고, Planned와 혼동하지 않도록 유지한다.
