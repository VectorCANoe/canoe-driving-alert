...
| IT_V2_FAILSAFE_001 | Req_127,Req_128,Req_129 | VC_127,VC_128,VC_129 | 경고 전달 이상 시 fail-safe 동작 통합 검증 | 수신·전달 이상 감지 후 `150ms` 이내 fail-safe 상태로 전환되고 자동 감속 보조는 차단되며 최소 경고 채널은 유지된다 (SIL Scenario 18) | Ready |  |  |
...
| IT_BACKBONE_STATE_001 | Req_166 | VC_166 | IBOX, SGW, DCM 등 경고 서비스 가용성 상태 통합 검증 | 관련 서비스 상태가 `100ms` 주기로 반영되고 경고 정보 전달 가용성과 강등 상태가 즉시 반영된다 | Ready |  |  |
