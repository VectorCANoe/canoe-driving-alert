...
| 확장성 | 신규 메시지를 위해 각 도메인별 예약 구간을 남겨둔다. |
...
| ID 대역 | 용도 | 대표 영역 |
|---|---|---|
| `0x100 ~ 0x13F` | 차량 기본 상태, 조향, 제동, 동력 기본 루프 | Chassis + Powertrain + 일부 Backbone |
| `0x1C0 ~ 0x20F` | 긴급 알림, 위험 판단, 주행 보조 상태 | ADAS + V2X |
| `0x260 ~ 0x27F` | 출입, 조명, 공조, 실내 편의 상태 | Body |
| `0x280 ~ 0x29F` | 내비 문맥, 클러스터 표시, HMI 상태 | IVI + CLU |
| `0x510 ~ 0x512` | Ethernet 경계 호환 메시지 | ETH seam compatibility |

## 5. 예약 구간

| ID 대역 | 용도 |
|---|---|
| `0x500 ~ 0x50F` | Ethernet 경계 확장 예약 |
| `0x513 ~ 0x53F` | Backbone/CGW 확장 예약 |
| `0x700 ~ 0x7FF` | Validation/Diagnostic 확장 예약 |

## 6. 배정 절차

1. 메시지의 Owner ECU와 도메인을 정한다.
2. 현재 활성 대역 또는 예약 구간에서 적절한 범위를 선택한다.
