...
| ECU 동작 | 교차로/합류 위험 경고 판정 | 교차로 측방 접근 및 합류/끼어들기 위험 경고를 생성하고 기존 경고와 정합 판정 | 객체 인지 확장 기능 | 교차로와 합류 위험 경고 확인 |
...
| ECU 동작 | 경고 서비스 가용성 상태 반영 | 백본 및 경고 서비스 가용성 상태를 통신 가용성과 서비스 강등 정책에 반영 | 확장 ECU 상태 연계 | 경고 서비스 가용성 반영 확인 |
