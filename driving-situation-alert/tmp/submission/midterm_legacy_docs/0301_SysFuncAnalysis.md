...
| 항목 | 확인 포인트 |
|---|---|
| 먼저 볼 구간 | `노드별 기능 명세(공식 표준 양식)` |
| 다음 확인 구간 | `Req-Func 대표 매핑` + `대표 체인 샘플` |
| 제출 관점 요약 | 노드 책임과 체인 추적의 일관성 |

## 중간보고 핵심

- 노드별 입력/처리/출력 책임을 기준으로 기능 경계를 명확히 제시한다.
- 검토 순서는 `노드별 기능 명세 -> Req-Func 대표 매핑`으로 고정한다.
- ECU 명칭은 Canonical 기준으로 유지해 문서 간 혼용을 방지한다.
