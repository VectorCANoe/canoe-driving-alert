...
| 항목 | 확인 포인트 |
|---|---|
| 먼저 볼 구간 | `시스템 변수 표(공식 표준 양식)` |
| 다음 확인 구간 | 하단 Var-Comm-Flow 대표 추적 구간 |
| 제출 관점 요약 | 변수 정의와 통신/구현 연결의 일관성 |

## 중간보고 핵심

- 변수는 입력/판정/출력 축으로 읽고 Comm/Flow 연결 키를 함께 확인한다.
- 검토 순서는 `시스템 변수 표 -> Var-Comm-Flow 대표 추적`으로 고정한다.
- 확장 변수는 Active/Pre-Activation 상태를 명시해 과대 해석을 방지한다.
