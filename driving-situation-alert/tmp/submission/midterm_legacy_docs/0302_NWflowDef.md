...
| 항목 | 확인 포인트 |
|---|---|
| 먼저 볼 구간 | `네트워크 플로우 표(공식 표준 양식)` |
| 다음 확인 구간 | `Flow SoT 요약` + `플로우 상세 추적 표` |
| 제출 관점 요약 | Flow 단위 경로와 상태(Defined/Implemented/Planned) |

## 중간보고 핵심

- Flow는 도메인 경계와 채널(CAN/ETH) 기준으로 경로를 확인한다.
- 검토 순서는 `네트워크 플로우 표 -> Flow SoT 요약`으로 고정한다.
- 상태값(Defined/Implemented/Planned)은 0303/0304와 동기화해 해석한다.
