...
- 11-bit 유지 조건:
  ...
  - 메시지 증가 추세가 단기(중간보고~최종보고) 범위에서 운영 가능한 수준이다.
  - 도메인 분리/우선순위 정책 설명이 11-bit 체계에서 충분히 디펜스 가능하다.
- 29-bit 전환 트리거(권고):
  - 특정 Tier/Group 슬롯 점유율이 지속적으로 높아(예: `>=80%`) 신규 배정 여유가 부족해질 때
  - 11-bit 공간에서 진단/확장 기능의 구조적 충돌을 해소하기 어려울 때
  - 외부 인터페이스/양산 연계 요구로 Extended ID가 명시 요구될 때
- 전환 원칙:
  - 전환 필요 시 정책(00f) -> 매핑표(Annex A) -> DBC/코드 -> 테스트 문서 순으로 단계 적용한다.
...
| 버전 | 날짜 | 변경 사항 |
|---|---|---|
| 3.7 | 2026-03-07 | 멘토 디펜스 보강: `11-bit 유지/29-bit 전환` 기준(유지 조건, 전환 트리거, 단계 적용 원칙)을 `5.5`에 추가해 선택형 권고 사항을 정책 문서에 명시. |
