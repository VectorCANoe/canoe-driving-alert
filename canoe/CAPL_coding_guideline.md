...
## 14. CANoe Write 창 한글 깨짐 현상

### 원인

CANoe Write 창은 **CP949 (한국어 Windows ANSI)** 코드페이지로 문자열을 표시한다.
.can 파일은 UTF-8로 저장되어 있어, 런타임에 출력된 UTF-8 바이트를 CP949로 해석 → 깨짐.

```
[Ambulance_Node] ?몃뱶 ?쒖옉 ...   ← UTF-8 한글이 CP949로 잘못 해석된 결과
[Ambulance_Node] start -- waiting  ← ASCII는 정상 표시
```

### Vector의 공식 입장

Vector CANoe는 공식적으로 **ANSI 인코딩** (시스템 코드페이지) .can 파일을 권장한다.
한국어 Windows에서는 CP949로 파일을 저장하면 한글이 Write 창에 올바르게 표시된다.

### 해결책 비교

| 방법 | Write 창 한글 | 컴파일 안전성 | 관리 복잡도 |
|------|-------------|-------------|-----------|
| **ASCII only in write()** (권장) | 영어 표시 (의도적) | 완전 안전 | 낮음 |
| .can 파일 CP949로 저장 | 한글 표시 | 확인 필요 | 높음 (Git, 에디터 설정 필요) |

### 권장: ASCII only

write() 출력은 디버깅용이므로 영어로 충분하다.
한글 정보는 주석(`/* */`)에 상세히 기록하면 소스 코드 가독성이 유지된다.
...
{
  /* 긴급 출동 활성화 시 keepAlive 브로드캐스트 시작 */
  write("[Police_Node] ** DISPATCH #%d broadcast start **", gPoliceDispatchCount);
}
```
