...
public class DoorFL_SetVariantCodingUSA
{
	...
	public void Execute()
	{
		...
		Tool.Output.WriteLine("Request: Default Session Start (STDS_RQ): 0x10 0x01");
		using (request = ecu.CreateRequest("DefaultSession_Start"))
		{
			  if(null != request)
			  {
			    DumpPdu(request.Pdu);
			    string ecuQualifier = ecu.Qualifier;
			    DumpParameters(request.Parameters);
			    Tool.Output.WriteLine("Sending Request 'DefaultSession_Start' to '"+ ecuQualifier +"' ...");
			    result = request.Send();
			    DumpResponses(result);
			  }
			  else
			  {
			    // Remove the exception below when the script shouldn't stop its execution
			    throw new Diag.DiagnosticException("Couldn't create request 'DefaultSession_Start'!");
			  }
		}
	
		Tool.Output.WriteLine("Trying to authenticate at ECU ...");
	
		Diag.AuthenticationResult authResult = ecu.Authenticate("Cert=UDS 0x29 Supplier;AutoSessionChange=1;");
		if(authResult == Diag.AuthenticationResult.Ok)
		{
		  Tool.Output.WriteLine("Authentication successful.");
		}
		else
		{
		   Tool.Output.WriteLine("Authentication failed.");
		}
		...
		//***************** Start manual modification		
		Tool.Output.WriteLine("Trying to deauthenticate at ECU ...");
	
		authResult = ecu.Deauthenticate();
		if(authResult == Diag.AuthenticationResult.Ok)
		{
		  Tool.Output.WriteLine("Deauthentication successful.");
		}
		else
		{
		   Tool.Output.WriteLine("Deauthentication failed.");
		}
		//***************** End manual modification
		#endregion
	}
	...
	private void DumpResponse(Diag.Response response)
	{
		  if (response != null)
		  {
		    // The Ecu property requires at least CANoe 9.0 SP3 / CANape 15 / CANoe4SW 18 SP3
