...
2020-12-11   1.16 Rue     Extensions for dynamic DO creation and DO containers
2020-12-22   1.17 Rue     Added GetDefaultOutParamsValue to VIAFbDOMethodProvider
2021-01-13   1.18 Hph     Added methods CreateGPBSerializationService, ReleaseGPBSerializationService, CreateJSONSerializationService, ReleaseJSONSerializationService
2021-04-23   1.19 Rue     Added SupportsFeature method to DO Binding Block interface and query methods to VIAFbDO
2021-06-22   1.20 Hag     Added SetDefaultOutParamsValue to VIAFbDOMethodProvider
2021-08-05   1.21 Aa      Added versioning for VIAFbService
2021-08-12   1.22 Hag     Added OnBindingVisibilityChanged to VIAFbDOBindingBlock
2021-08-20   1.23 Hag     Added VIAFbDOEmbeddedObjectContainerMember
2021-10-06   1.24 Hag     Added VIAFbDOEmbeddedObject, added GetEmbeddedObject to VIAFbDOContainer
2021-11-02   1.25 Aa      Added offline feature
2021-11-10   1.26 Srj     Added monitoring feature
2021-11-16   1.27 Srj     Extended member and distObj with monitoring status
2022-01-10   1.28 Wan     Added VIAFbCommonBindingService
2021-11-29   1.29 Srj     Monitoring values have to be writable
2022-05-10   1.30 Hag     Added AttributeAccess to VIAFbType for struct-members
2023-09-19   1.31 Jsr     Added method Clone to VIAFbValue
2023-12-08   1.32 Rue     Added Begin/EndValueBulkUpdate to VIAFbDOValueProvider
-----------------------------------------------------------------------------*/

#define VIAFunctionBusServiceMajorVersion 1

#ifndef VIAFunctionBusServiceMinorVersion
#define VIAFunctionBusServiceMinorVersion 32
#endif
...
class VIAFbDOEmbeddedObjectContainerMember;
class VIAFbDOEmbeddedObject;
...
class VIAFbGPBSerializationService;
class VIAFbJSONSerializationService;
...
class VIAFbStatus;
class VIAFbType;
class VIAFbCommonBindingService;
...
enum VIAFbDOMemberInterfaceType
{
  ...
  // container member
  eFbDOMemberIF_EmbeddedObjectContainer,
  eFbDOMemberIF_EmbeddedObject,
};
...
#if defined(VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 18)

  //Create a service object for Google Protocol Buffers (de-)serialization
  //[OUT] kVIA_OK if successful, else kVIA_Failed
  VIASTDDECL CreateGPBSerializationService(VIAFbGPBSerializationService ** GPBSerializationService) = 0;

  //Delete a service object for Google Protocol Buffer (de-)serialization
  //[OUT] kVIA_OK if successful
  VIASTDDECL ReleaseGPBSerializationService(VIAFbGPBSerializationService ** GPBSerializationService) = 0;

  //Create a service object for JSON(de-)serialization
  //[OUT] kVIA_OK if successful, else kVIA_Failed
  VIASTDDECL CreateJSONSerializationService(VIAFbJSONSerializationService ** JSONSerializationService) = 0;

  //Delete a service object for JSON (de-)serialization
  //[OUT] kVIA_OK if successful
  VIASTDDECL ReleaseJSONSerializationService(VIAFbJSONSerializationService ** JSONSerializationService) = 0;

#endif

#if defined(VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 21)
  // Gets the supported version of the CANoe, i.e. returns VIAFunctionBusServiceMajorVersion and VIAFunctionBusServiceMinorVersion
  // Always returns kVIA_OK
  VIASTDDECL GetVersion(int32* major, int32* minor) = 0;
#endif

#if defined (VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 28)
  // Create a service object for accessing Common Binding data model artifacts in the requested version.
  // Returns kVIA_OK if successful.
  VIASTDDECL GetFbCommonBindingService(VIAFbCommonBindingService** ppFbCommonBindingService, int32 majorVersion, int32 minorVersion) = 0;

  // Releases the Common Binding Service.
  // Returns kVIA_OK if successful.
  VIASTDDECL ReleaseFbCommonBindingService(VIAFbCommonBindingService* pFbCommonBindingService) = 0;
#endif
};
...
// VIA interface for DOs and also for the DO part of embedded objects
class VIAFbDO
{
public:
...
#if defined(VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 19)
  // Retrieve the name of this DO
  ...
  // Retrieve the full DO path (including namespaces)
  // [RETURN] kVIA_OK if successful, kVIA_BufferToSmall if buffer too small, kVIA_ParameterInvalid in case of a bad in parameter
  // [THREADSAFE] Yes
  VIASTDDECL GetFullPath(char* inBuffer, size_t inBufferLength) const = 0;
#endif

#if defined(VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 27)
  // Retrieve the monitoring status of this DO
  // [RETURN] kVIA_OK if successful, kVIA_ParameterInvalid in case of a bad in parameter
  // [THREADSAFE] Yes
  VIASTDDECL IsMonitoringEnabled(bool* outEnabled) const = 0;
#endif
};
...
// A container for distributed objects, DO references or embedded objects
class VIAFbDOContainer
{
public:
  // Releases the object
  ...
  // For leaf containers of DOs or embedded objects: adds an element at the end of the container, enlarging the container
  ...
  // For leaf containers of DOs or embedded objects: destroys the element a the specified index
  // [IN] index of the DO or embedded object
  // [RETURN] kVIA_OK; kVIA_Failed if index is invalid
  // [THREADSAFE] No
  VIASTDDECL DestroyDOElement(uint32 index) = 0;
  // For leaf containers of DOs or embedded objects: recreates the element at the specified index. If there is already an element at the index,
...
#if defined(VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 24)
  // For leaf containers of Embedded DOs: returns the element at the specified index.
  // [IN] index of the embedded DO
  // [OUT] the embedded DO at the index or null if there is no embedded DO at the index
  // [RETURN] kVIA_OK; kVIA_Failed if index is invalid; kVIA_ParameterInvalid if embedded DO parameter is nullptr
  // [THREADSAFE] No
  VIASTDDECL GetEmbeddedObjectElement(uint32 index, VIAFbDOEmbeddedObject** outEmbeddedObject) const = 0;
#endif
};
...
#if defined(VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 27)
  // Retrieve the monitoring status of this Member
  // [RETURN] kVIA_OK if successful, kVIA_ParameterInvalid in case of a bad in parameter
  // [THREADSAFE] Yes
  VIASTDDECL IsMonitoringEnabled(bool* outEnabled) const = 0;
#endif
...
public:
...
#if defined(VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 20)
  // Set the default output arguments of this DO member's next automatic response.
  ...
  // [THREADSAFE] No
  VIASTDDECL SetDefaultOutParamsValue(
...
#if defined(VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 32)
  // Begin a bulk update of the DO member. Note: you don't need normally need this, just change the VIAFbValue gradually and
  // call SetValue once at the end. The methods are used by the C library implementation.
  // [OUT] kVIA_OK if successful, kVIA_ObjectInvalid in case of an invalid object
  // [THREADSAFE] No
  VIASTDDECL BeginValueBulkUpdate() = 0;
  // End a bulk update of the DO member. There must be as many end calls as there are begin calls. On the last end call,
  // the new value is propagated.
  // Note: you don't need normally need this, just change the VIAFbValue gradually and
  // call SetValue once at the end. The methods are used by the C library implementation.
  // [OUT] kVIA_OK if successful, kVIA_ObjectInvalid in case of an invalid object
  // [THREADSAFE] No
  VIASTDDECL EndValueBulkUpdate() = 0;
#endif
};
...
#if defined(VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 23)

class VIAFbDOEmbeddedObjectContainerMember : public VIAFbDOMemberInterface
{
public:
  enum MemberInterfaceType { InterfaceType = eFbDOMemberIF_EmbeddedObjectContainer };

  // Retrieve root embedded object container.
  // [OUT] kVIA_OK if the query was successful, kVIA_ParameterInvalid in case of a bad parameter, kVIA_Failed if the container could not be fetched
  // [THREADSAFE] No
  VIASTDDECL GetRootContainer(VIAFbDOContainer** outRootContainer) = 0;
};
#endif

#if defined(VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 24)
class VIAFbDOEmbeddedObject : public VIAFbDOMemberInterface
{
public:
  enum MemberInterfaceType { InterfaceType = eFbDOMemberIF_EmbeddedObject };

  // Get DO part of this embedded object.
  // [OUT] kVIA_OK if the query was successful, kVIA_ParameterInvalid in case of a bad parameter, kVIA_Failed if the VIAFbDO could not be fetched
  // [THREADSAFE] No
  VIASTDDECL GetMemberAsVIAFbDO(VIAFbDO** pVIAFbDO) = 0;
};
#endif
...
#if defined (VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 29)
  // Obtain a value interface for the subscription state
  // [OUT] kVIA_OK if parameters could be retrieved, kVIA_ParameterInvalid/kVIA_ObjectInvalid in case of a bad parameter or handle, kVIA_Failed in case of an error
  // [THREADSAFE] No
  VIASTDDECL GetSubscriptionStateValue(VIAFbDOValue** stateValue) const = 0;
#endif
};
...
#if defined (VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 29)
  // Obtain a value interface for the announcement state
  // [OUT] kVIA_OK if parameters could be retrieved, kVIA_ParameterInvalid/kVIA_ObjectInvalid in case of a bad parameter or handle, kVIA_Failed in case of an error
  // [THREADSAFE] No
  VIASTDDECL GetAnnouncementStateValue(VIAFbDOValue** stateValue) const = 0;
#endif
};
...
#if defined(VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 31)
  // Clones the current value by creating a new independent value
  // [OUT] kVIA_FunctionNotImplemented if not cloneable (e.g. already an independent value), kVIA_OK otherwise  
  // [THREADSAFE] No
  VIASTDDECL Clone(
    VIAFbValue** outValue) const = 0;                           // [OUT] The value object
#endif
};
...
#if defined(VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 30)
  // Access to attributes for struct members.
  // Returns kVIA_OK, kVIA_ParameterInvalid (if given pointer is nullptr) or kVIA_Failed in case of error.
  // [OUT] Pointer receiving the attribute access object. The object must be
  // deleted by calling Release on it.
  // [THREADSAFE] No
  VIASTDDECL GetMemberAttributeAccess(VIAFbTypeMemberHandle inMemberHandle,   // [IN] Handle to a sub-member of this composite type
    VIAFbAttributeAccess** outAttributeAccess) const = 0;                     // [OUT] Attribute access of this particular member
#endif
};
...
#if defined(VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 19)
enum DOBindingBlockFeature
{
  DOBindingBlockFeature_DOInterfaceDataTypes = 0, // binding supports DO interfaces as data types
  DOBindingBlockFeature_DOOfflineMode        = 1, // binding supports DO offline mode
  DOBindingBlockFeature_DOMonitoring         = 2, // binding supports DO monitoring mode
};
#endif
...
#endif
#if defined(VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 19)
  VIASTDDECL SupportsFeature(DOBindingBlockFeature feature, bool* outFeatureSupported) = 0;
#endif
#if defined(VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 19)
  VIASTDDECL OnBindingVisibilityChanged(bool inBindingVisibility) = 0;
#endif
#if defined(VIAFunctionBusServiceMinorVersion) && (VIAFunctionBusServiceMinorVersion >= 25)
  // Called before OnMeasurementInit to specify online or offline mode
  // Only useful if the binding supports feature 'DOBindingBlockFeature_DOOfflineMode'
  VIASTDDECL SetOnlineOfflineMode(bool isOnline) = 0;
#endif
};
