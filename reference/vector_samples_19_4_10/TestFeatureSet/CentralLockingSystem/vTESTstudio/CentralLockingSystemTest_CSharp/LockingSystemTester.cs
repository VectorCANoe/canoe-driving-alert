...
public class MyTestClass
{
  ...
  public static void TC_LockStatically()
  {
    ...
    // Reset the system
    Report.TestStep("Reset the system...");
    LockingTestLibrary.ResetLockingTestSignals();
    LockRequest.Value = LockRequest.Request_unlock;
    Execution.Wait(SysPars.LockingTests.LockRequestWaitTime.GetValue());
  ...
  public static void TC_UnlockStatically()
  {
    ...
    Report.TestStep("Check unlock function when engine is off...");
    EngineRunning.Value = 0;
    LockRequest.Value = LockRequest.Request_unlock;
    Execution.Wait(SysPars.LockingTests.LockRequestWaitTime.GetValue());
    ...
    Report.TestStep("Check unlock function when engine is on...");
    EngineRunning.Value = 1;
  ...
  /// <summary>
	/// Checks the crash detection of the car while the engine is off and the car is not moving
	/// </summary>
  [Export]
  [TestCase]
  ...
  [Export]
  [TestCase]
  public static void TC_LockByIncreasingVelocity(double maxVelocity = 60)
  {
    // Initialize the system
    Report.TestStep("Initialize the system...");
    LockingTestLibrary.ResetLockingTestSignals();
    ...
    // Execute tests for locking by velocity
    ...
    CaplTestFunctions.Utilities.AccelerateDecelerateToTargetSpeed(maxVelocity);
    Execution.Wait(500);
  ...
  public static void TC_AmbiguousKeyUpDown(int engineRunning)
  {
    ...
    // Retrieve the ECU by querying the application object
