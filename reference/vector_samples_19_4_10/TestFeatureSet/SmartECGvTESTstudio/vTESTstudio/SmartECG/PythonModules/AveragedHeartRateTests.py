...
class HeartRateObserver:
  ...
  def compare_average_heart_rate(self):
    ...
    if av_heart_rate_offset >= len(reference_av_heart_rate_array):
      failing_heart_rate_deviation = av_hr_deviation
      heart_rate_observation_status = HeartRateObservationStatus.MEASURED_HEART_RATE_UNEXPECTED
      return

    av_hr_deviation = abs(SmartECG.SmartECG_IO.averagedHeartRate.copy() - reference_av_heart_rate_array[av_heart_rate_offset])
    if av_heart_rate_offset > sys_pars.InitializationRelatedNumberOfHeartRates.copy() and av_hr_deviation > sys_pars.HeartRateDeviationThreshold.copy():
      failing_heart_rate_deviation = av_hr_deviation 
    ...
    if av_heart_rate_offset - sys_pars.InitializationRelatedNumberOfHeartRates.copy() + 1 == sys_pars.NumberOfRequiredCorrectAvHeartRates.copy():
      if sample_timer is not None:
        sample_timer.cancel()
...
def get_time_between_samples_ms() -> int:
  time_between_samples_in_ms = 1000 / sys_pars.InputFrequency.copy()
