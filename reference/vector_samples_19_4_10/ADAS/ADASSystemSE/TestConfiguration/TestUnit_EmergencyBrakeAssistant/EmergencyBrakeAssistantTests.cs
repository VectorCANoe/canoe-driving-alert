...
public class BrakeAssistantTests
{
    ...
    {
        if (Execution.WaitForUpdate(ADAS.FrontRadar.sensor_info.detected_objects, 1000) == 1)
        {
    ...
    public static void IsExistenceProbabilityOver70Percent()
    {
        ...
        if (detectedObject.moving_object.header.Value.existence_probability.Value.ImplValue >= 0.7)
        {
        ...
        else
        {
            ...
            {
                if (Execution.WaitForUpdate(detectedObject.moving_object.header.Value.existence_probability.Value, 100) == 1) // 0 = Timeout, 1 = Success, -1 = Fail
                {
                    if (detectedObject.moving_object.header.Value.existence_probability.Value.ImplValue >= 0.7)
                    {
                        Report.TestStepPass("Success, the Existence Probability was over 70%");
    ...
    public static void IsRadarObjectInSameLane()
    {
        ...
        while (executionTime <= 60000)
        {
            if (Execution.WaitForUpdate(detectedObject.moving_object.baseInfo.Value.position.Value.y.Value, 100) == 1) // 0 = Timeout, 1 = Success, -1 = Fail
            {
                if (detectedObject.moving_object.baseInfo.Value.position.Value.y.Value <= 0.25 &&
                    detectedObject.moving_object.baseInfo.Value.position.Value.y.Value >= -0.25)
                {
                    success = true;
                    break;
                }
    ...
    [Export]
    [TestCase("No Warning when TTI > 3000ms", "Test if no warning is active when the time to impact is bigger than 3000ms")]
    public static void NoWarningWhenTtiOver3000ms()
    {
        double timeToImpact = 99999;
        int isWarningActive = 0;

        timeToImpact = ADAS.OutputSimpleAlgorithm.timeToImpact.Value;
        isWarningActive = ADAS.OutputSimpleAlgorithm.isWarningActive.Value;

        if (timeToImpact > 3000)
        {
            if (isWarningActive == 0)  // ok
            {
                Report.TestStepPass("Success, no warning was provided as expected.");
            }
            else // nok
            {
                Report.TestStepFail("Fail, the TTI (" + timeToImpact.ToString() + ") was too high, but a warning was provided anyways.");
            }
        }
    }

    [Export]
    [TestCase("Warning when TTI <= 3000ms", "Test if a warning is active when the time to impact is smaller or equal to 3000ms")]
    public static void WarningWhenTtiSmaller3000ms()
    {
        double timeToImpact = 99999;
        int isWarningActive = 0;
        ...
        while (executionTime <= 30000)
        {
            ...
            if (timeToImpact <= 3000)
            {
                if (isWarningActive == 1)  // ok
                {
                    Report.TestStepPass("Success, a warning was provided when necessary.");
                    break;
                }
                else // nok
                {
                    Report.TestStepFail("Fail, the TTI (" + timeToImpact.ToString() + ") indicates that a warning is necessary, but no warning was provided.");
                    break;
                }
            }
            else
