...
class PrimitiveInt:
    ...
    def deserialize(self, deserializer, stored_raw) -> int:
        if self.signed:
            return deserializer.DeserializeInt64(self.bitSize)
        else:
            return deserializer.DeserializeUint64(self.bitSize)
...
class EnumDataType(Generic[T]):
    def __init__(self, enumType, initialValue, signed, bitSize):
    ...
    def serialize(self, serializer, value, stored_raw):
        if isinstance(value, self._pythonDataType):
            if self.signed:
                ...
                serializer.SerializeUint64(value.value, self.bit_size)
        else:
            raise ValueError(f"{value} is not of type {self._pythonDataType.__name__}.")
    ...
    def deserialize(self, deserializer, stored_raw) -> T:
        if len(self._pythonDataType) > 0:
            if self.signed:
                ...
                return self._pythonDataType(deserializer.DeserializeUint64(self.bit_size))
        else:
            raise ReadEmptyEnumError(f"Cannot read value for enum without elements: {self._pythonDataType.__name__}.")
...
class UnionType(Generic[T]):
    ...
    def _serialize(actual_type: Callable, serializer, val, stored_raw):
        if val.discriminator is None:
            serializer.BeginUnion(0)
        else:
    ...
    def _deserialize_in_place(actual_type, discriminator_type, deserializer, value, stored_raw):
        discriminator_value = deserializer.BeginUnion()
        if discriminator_value == 0:
            value._discriminator = None
            value._value = None
        else:
            value._discriminator = discriminator_type(discriminator_value)
...
class Array_Base(Generic[T], UserList):
    ...
    def __getitem__(self, i) -> T:
        if (i >= self._num_elements):
            raise IndexError(f"Index {i} is out of bounds for array of size {self._num_elements}.")
    ...
    def extend(self, iterable: Iterable[T]):
        raise TypeError("Extend is not available for array types.")

    def insert(self, i, item):
        raise TypeError("Insert is not available for array types.")

    def pop(self, i=-1):
        raise TypeError("Pop is not available for array types.")

    def clear(self):
        raise TypeError("Clear is not available for array types.")

    def remove(self, item):
        raise TypeError("Remove is not available for array types.")

    def __delitem__(self, i):
        raise TypeError("Deleting an item is not possible for array types.")

    def __iadd__(self, other):
        raise TypeError("Adding items implicitly is not possible for array types.")

    def __imul__(self, n):
        raise TypeError("Implicit multiplying is not possible for array types.")
    ...
    def _serialize(serializer, values, element_dt, stored_raw):
        ...
        for i in range(count):
            element_dt.serialize(serializer, values[i], stored_raw)
        serializer.EndArray()
...
class SVLegacyArray_Base(Generic[T], UserList):
    def __init__(self, num_elements, element_dt):
        self._num_elements = num_elements
        self._element_dt = element_dt
        super().__init__([self._element_dt.initial_value() for k in range(self._num_elements)])
    ...
    def _serialize(serializer, values, element_dt, stored_raw):
        ...
        if isinstance(element_dt, DoubleDataType):
          serializer.SerializeSVLegacyDoubleArray(array)
        else:
          serializer.SerializeSVLegacyIntArray(array)

    @staticmethod
    def _deserialize(deserializer, array_dt, element_dt, stored_raw):
        newValue = array_dt()
        SVLegacyArray_Base._deserialize_in_place(deserializer, element_dt, newValue, stored_raw)
        return newValue

    @staticmethod
    def _deserialize_in_place(deserializer, element_dt, value, stored_raw):
        if isinstance(element_dt, DoubleDataType):
          values = deserializer.DeserializeSVLegacyDoubleArray()
          for i in range(0, len(values)):
            value[i] = values[i]
        else:
          values = deserializer.DeserializeSVLegacyIntArray()
          for i in range(0, len(values)):
            value[i] = values[i]
...
class List_Base(Generic[T], UserList):
    ...
    def append(self, item: T):
        if self._has_conversion:
            raise TypeError("Append is not supported for encoded lists. Use resize() instead.")
        else:
            if(self._max_size == 0 or len(self.data) < self._max_size):
                super().append(item)
            else:
                raise IndexError(f"Max. size {self._max_size} of list is reached.")

    def extend(self, iterable: Iterable[T]):
        if self._has_conversion:
            raise TypeError("Extend is not supported for encoded lists. Use resize() instead.")
        else:
            if(self._max_size == 0 or len(self.data) + len(iterable) <= self._max_size):
    ...
    def insert(self, i, item: T):
        if self._has_conversion:
            raise TypeError("Insert is not supported for encoded lists. Use resize() instead.")
        else:
            if(self._max_size == 0 or len(self.data) < self._max_size):
                super().insert(i, item)
            else:
    ...
    def remove(self, item: T):
        if self._has_conversion:
            raise TypeError("Remove is not supported for encoded lists. Use resize() instead.")
        else:
            if len(self.data) > self._min_size:
                super().remove(item)
            else:
                raise IndexError(f"Min. size {self._min_size} of list is reached.")
    ...
    def resize(self, count):
        if(self._min_size <= count and (self._max_size == 0 or self._max_size >= count)):
            ...
            max_size_str = ('' if self._max_size == 0 else f' and max. size {self._max_size}')
            raise ValueError(f"Count {count} is out of range for list with min. size {self._min_size}{max_size_str}.")

    def __getitem__(self, i) -> T:
        if (i >= len(self.data)):
            raise IndexError(f"Index {i} is out of bounds for list of size {len(self.data)}.")
        return super().__getitem__(i)
    ...
    def __iadd__(self, other):
        if (self._max_size > 0 and len(self.data) + len(other) > self._max_size):
            raise IndexError(f"Max. size {self._max_size} of list is reached.")
        return super().__iadd__(other)

    def __imul__(self, n):
        if (n == 0 and self._min_size > 0):
            raise IndexError(f"Min. size {self._min_size} of list is reached.")
        if (self._max_size > 0 and n * len(self.data) > self._max_size):
            raise IndexError(f"Max. size {self._max_size} of list is reached.")
        return super().__imul__(n)
    
    def pop(self, i=-1):
        if (len(self.data) - 1 < self._min_size):
            raise IndexError(f"Min. size {self._min_size} of list is reached.")
        return super().pop(i)
...
def makeConcreteDataTypeWithConversionCommon(implConv, rawConv, implDt, physDt, rawDt, symbDt, BaseType):
    ...
    class ConcreteDataTypeWithConversion(BaseType):
        ...
        @staticmethod
        def convertToSerializable(value):
            '''
            For values passed as unwrapped Python data types as for example elements of a list.
            '''
            if isinstance(value, DataTypeWithConversion):
                return value
            else:
                serializable = ConcreteDataTypeWithConversion.initial_value()
                if isinstance(value, DataTypeWithConversionPhys) or isinstance(value, DataTypeWithConversionPhysRaw):
                    serializable._phys_value = value
                else:
                    serializable._value = value
                return serializable

        @staticmethod
        def serialize(serializer, val, stored_raw): 
            val = ConcreteDataTypeWithConversion.convertToSerializable(val)
