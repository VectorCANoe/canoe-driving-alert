...
public class SensorValueGenerator : MeasurementScript
{
    ...
    [OnChange(ExploratoryTesting.Panel.MemberIDs.GenerateSensorValuesActive)]
    public void OnAutomaticModeChange()
    {
        if (ExploratoryTesting.Panel.GenerateSensorValuesActive.Value)
        {
            ResetSensorValues();
            _timer.Start();
        }
