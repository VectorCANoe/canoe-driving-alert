function(select_canoe_install_dir)
    # Check if sil-adapter-builder.exe is available on PATH
    find_program(SIL_ADAPTER_BUILDER_EXE "sil-adapter-builder.exe")
    if(SIL_ADAPTER_BUILDER_EXE)
        get_filename_component(EXEC64_DIR ${SIL_ADAPTER_BUILDER_EXE} DIRECTORY)
        set(SIL_ADAPTER_SIL_ADAPTER_BUILDER_DIR ${EXEC64_DIR} PARENT_SCOPE)
        unset(SIL_ADAPTER_BUILDER_EXE CACHE)
        return()
    endif()

    # Consider environment vars for finding CANoe install dir
    set(relevantEnvVars "CANoe_InstallDir;CANoe_InstallDir64;CANoeMedTech_InstallDir;CANoeMedTech_InstallDir64;CANoe4SW_InstallDir;CANoe4SW_InstallDir64;CANoe4SWLite_InstallDir;CANoe4SWLite_InstallDir64;CANoeSE_InstallDir;CANoeSE_InstallDir64;CANoe4SWSE_InstallDir;CANoe4SWSE_InstallDir64;TestUnitRunner_InstallDir64")
...
function(get_dependent_file_list)
    ...
    set(buildDir "${CMAKE_CURRENT_BINARY_DIR}/.${SIL_ADAPTER_NAME}.build")

    set(dependentFileListSearchPaths
        "${buildDir}/${SIL_ADAPTER_NAME}.venvironment"
        "${buildDir}/${SIL_ADAPTER_NAME}.venvironment-basic"
        "${buildDir}/${SIL_ADAPTER_NAME}.venvironment/.build"
        "${buildDir}/${SIL_ADAPTER_NAME}.venvironment-basic/.build"
    )

    foreach(searchPath IN LISTS dependentFileListSearchPaths)
        if(EXISTS "${searchPath}/dependent_file_lists.json")
            set(dependentFileListPath "${searchPath}/dependent_file_lists.json")
        endif()
    endforeach()

    if(NOT dependentFileListPath)
        message(WARNING "Couldn't find 'dependent_file_lists.json'. Automatic rerun of CMake is disabled if a .vCDL file is changed. Consider updating the SilAdapter.cmake script. The script can be found in the SIL Adapter sample configurations.")
        set(DEPENDENT_FILE_LIST "" PARENT_SCOPE)
        return()
    endif()
...
function(add_siladapter)
    cmake_parse_arguments("SIL_ADAPTER" "" "NAME;OUTPUT_DIR;SIL_ADAPTER_BUILDER_DIR;VERBOSITY_LEVEL;LANGUAGE" "INPUT_VCDLS" ${ARGN} )
    ...
    if(NOT DEFINED SIL_ADAPTER_LANGUAGE)
        set(SIL_ADAPTER_LANGUAGE "Cpp")
    endif()
    ...
    execute_process(COMMAND "${SIL_ADAPTER_SIL_ADAPTER_BUILDER_DIR}/sil-adapter-builder.exe" ${RESOLVED_INPUT_SOURCES} -l ${SIL_ADAPTER_LANGUAGE} -o ${SIL_ADAPTER_OUTPUT_DIR} -n ${SIL_ADAPTER_NAME} -t ${CMAKE_CURRENT_BINARY_DIR} -v ${SIL_ADAPTER_VERBOSITY_LEVEL} RESULT_VARIABLE SAB_RESULT)
    ...
    if(DEFINED SIL_ADAPTER_INPUT_VCDLS)
        ...
        foreach(INPUT_VCDL ${DEPENDENT_FILE_LIST})
            ...
            set_property(DIRECTORY APPEND PROPERTY CMAKE_CONFIGURE_DEPENDS "${INPUT_VCDL}")
        endforeach()
    endif()
