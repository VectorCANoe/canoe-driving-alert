...
[그림 1 삽입 위치: V2X 긴급 이벤트 선택 및 timeout clear 상태도]
...
객체 기반 위험도는 TTC(Time-to-Collision)와 객체 신뢰도, 거리, 시나리오 편향을 함께 반영하였다. 상대 속도를 \(v_{rel}\), 객체 거리를 \(d\)라 하면 TTC는
...
[그림 2 삽입 위치: ADAS 위험도 산정 및 계층형 경고 중재 블록도]
...
[그림 3 삽입 위치: CAN/Ethernet 혼합 경고 전달 구조도]
...
[그림 4 삽입 위치: CANoe SIL 구현 및 시험 구성도]
