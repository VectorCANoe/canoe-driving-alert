...
\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{asset_36_v2x_role_normalization.png}
\caption{V2X 긴급 이벤트 정규화 구조}
\label{fig:v2x_state}
\end{figure*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{asset_24_adas_state_function.png}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{asset_33_fail_safe_boundary_recovery.png}
\caption{Fail-Safe 및 복귀 규칙}
\label{fig:fail_safe_recovery}
\end{figure*}
...
\subsection{핵심 seam 권한 구조}

혼합 네트워크 구조가 설명 가능하려면 메시지 자체보다 먼저 ``어떤 seam을 누가 소유하는가''가 정리되어야 한다. 본 연구에서는 owner, primary bus, timeout authority, route authority를 분리하여 정의함으로써, 기능 의미와 경계 정책을 transport와 독립적으로 설명할 수 있도록 하였다. 특히 긴급 경고와 감속 보조는 ADAS가 판단하되, 최종 effective result와 fail-safe shaping은 CGW가 담당하도록 하여, 기능 판단과 경계 적용이 섞이지 않도록 구성하였다~\cite{comm_spec,sysvar_spec}.

\begin{table*}[t]
\caption{핵심 seam 권한 구조 요약}
\label{tab:seam_authority}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.4cm} >{\raggedright\arraybackslash}p{1.5cm} >{\raggedright\arraybackslash}p{2.2cm} >{\raggedright\arraybackslash}p{2.0cm} >{\raggedright\arraybackslash}p{1.6cm} >{\raggedright\arraybackslash}X}
\toprule
...
Emergency context & V2X & Ethernet + CoreState & CGW & CGW & 긴급차량 유형, 방향, ETA, SourceID \\
Arbitration decision & ADAS & local runtime & ADAS & CGW & selected alert 결정 전 상태 \\
Arbitration effective result & CGW & effective seam & CGW & CGW & selected alert 경계 적용 후 상태 \\
Decel decision & ADAS & local runtime & ADAS & CGW & 감속 보조 판단과 driver release reason \\
Decel effective result & CGW & effective seam & CGW & CGW & fail-safe 적용 후 최종 감속 보조 \\
\bottomrule
\end{tabularx}
\end{table*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{asset_16_network_message_flow.png}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{asset_23_diag_observability_layer.png}
\caption{핵심 경고 판단과 진단·관측 계층 구성}
\label{fig:canoe_impl}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{asset_35_panel_input_readback_contract.png}
\caption{패널 입력과 상태 확인 구조}
...
정량 지표의 집계 단위는 개별 신호나 개별 테스트 코드가 아니라 ``시나리오 단위 verdict''로 두었다. 이는 동일 경고 컨셉이 단위시험, 통합시험, 시스템시험에서 반복적으로 등장하더라도, 최종 해석은 기능 의미 단위로 수행해야 한다는 판단에 따른 것이다. 또한 각 지표는 owner-published seam과 시험 verdict를 함께 사용하도록 정의함으로써, 단순 PASS 수보다 기능 의미와 경계 조건을 동시에 읽을 수 있도록 하였다~\cite{comm_spec,master_test_matrix,system_test,sysvar_spec,suite_policy}.
...
\begin{table*}[t]
\caption{정량 지표와 데이터 출처}
\label{tab:metric_sources}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3.0cm} >{\centering\arraybackslash}p{3.0cm} >{\raggedright\arraybackslash}p{1.9cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{2.8cm}}
\toprule
지표 & 집계식 & 판정 단위 & 주요 데이터 출처 & 대표 시험 \\
\midrule
V2X 긴급 이벤트 선택 정확도 & $N1=(S_{sel}/T_{v2x})\times100$ & 시나리오 & emergencyType, EtaSeconds, SourceId, selected alert verdict & IT\_006, ST\_016, ST\_017 \\
교차로·합류구간 통합 경고 성공률 & $N2=(S_{int}/T_{int})\times100$ & 시나리오 & objectTtcMin, objectRiskClass, decelAssistReq, selected alert verdict & IT\_018, ST\_022, ST\_023 \\
복귀 안정성 준수율 & $N3=(S_{rec}/T_{rec})\times100$ & 시나리오 & timeoutClear, warningPathStatus, effective alert state, baseline verdict & IT\_009, ST\_035, ST\_036, ST\_045, ST\_046 \\
\bottomrule
\end{tabularx}
\end{table*}
...
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3.0cm} >{\centering\arraybackslash}p{3.0cm} >{\raggedright\arraybackslash}X >{\centering\arraybackslash}p{1.3cm}}
\toprule
지표 & 계산식 & 해석 & 값(\%) \\
\midrule
...
\bibitem{func_def}
현대모비스 부트캠프 벡캔팀, ``03\_Function\_definition Excel,'' 버전 4.39, 2026-03-17.

\bibitem{integration_test}
현대모비스 부트캠프 벡캔팀, ``06\_Integration\_Test Excel,'' 버전 4.23, 2026-03-17.

\bibitem{comm_spec}
현대모비스 부트캠프 벡캔팀, ``0303\_Communication\_Specification Excel,'' 버전 3.36, 2026-03-17.

\bibitem{hara}
현대모비스 부트캠프 벡캔팀, ``00d\_HARA\_Worksheet Excel,'' 버전 1.6, 2026-03-11.

\bibitem{master_test_matrix}
현대모비스 부트캠프 벡캔팀, ``Test Matrix (00g\_Master\_Test\_Matrix Excel),'' 버전 0.2, 2026-03-15.

\bibitem{system_test}
현대모비스 부트캠프 벡캔팀, ``07\_System\_Test Excel,'' 버전 5.23, 2026-03-17.

\bibitem{sw_impl}
현대모비스 부트캠프 벡캔팀, ``04\_SW\_Implementation Excel,'' 버전 2.26, 2026-03-17.

\bibitem{unit_test}
현대모비스 부트캠프 벡캔팀, ``05\_Unit\_Test Excel,'' 버전 2.26, 2026-03-17.

\bibitem{nw_flow}
현대모비스 부트캠프 벡캔팀, ``0302\_NWflowDef Excel,'' 버전 3.31, 2026-03-17.
...
현대모비스 부트캠프 벡캔팀, ``00\_Project\_Overview Excel,'' 버전 0.5, 2026-03-17.

\bibitem{concept_design}
현대모비스 부트캠프 벡캔팀, ``02\_Concept\_design Excel,'' 버전 0.5, 2026-03-17.

\bibitem{sysfunc_analysis}
현대모비스 부트캠프 벡캔팀, ``0301\_SysFuncAnalysis Excel,'' 버전 3.16, 2026-03-17.

\bibitem{sysvar_spec}
현대모비스 부트캠프 벡캔팀, ``0304\_System\_Variables Excel,'' 버전 3.10, 2026-03-17.
...
현대모비스 부트캠프 벡캔팀, ``CANoe test suites 운영 기준,'' 2026-03-21.
