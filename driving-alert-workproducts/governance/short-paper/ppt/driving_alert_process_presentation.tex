...
\subtitle{구간 정보 및 긴급차량 접근 기반 경고 컨셉\\설계, 구현, 시험, 운영 자동화 경험}
...
\begin{frame}{발표 개요}
\begin{enumerate}
  \item 왜 이 프로젝트를 시작했는가
  \item 어떤 컨셉과 문서 체계를 선택했는가
  \item 어떤 ECU 구조와 네트워크 흐름으로 구현했는가
  \item 어떤 시험 체계와 운영 구조를 구축했는가
  \item 무엇이 도출되었고 무엇을 배웠는가
\end{enumerate}
\end{frame}
...
\begin{itemize}
  \item 본 프로젝트는 차량 경고 기능을 하나 더 추가하는 과제가 아니라, 서로 다른 경고 원인을 하나의 정책으로 통합하는 과제에서 출발하였다.
  \item 실제 차량 맥락에서는 구간 정보, 긴급차량 접근, 객체 위험이 동시에 발생할 수 있으며, 이 경우 개별 기능은 쉽게 충돌한다.
  \item 따라서 문제의 핵심은 센서 추가가 아니라, 다중 입력을 단일 경고와 단일 복귀 규칙으로 해석하는 구조를 만드는 데 있었다.
\end{itemize}
\end{frame}

\begin{frame}{현장에서 먼저 보인 문제}
\begin{itemize}
  \item 구간 경고와 긴급 경고가 동시에 유효해도 최종 경고가 일관되게 유지되지 않는 경우가 있었다.
  \item 경고 시작, 전환, 종료 과정에서 표시 중복과 복귀 불안정이 반복되었다.
  \item 동일 기능이 CAN과 Ethernet 경계를 지나면서 설계 의도와 시험 기대값이 쉽게 분리되는 문제가 나타났다.
\end{itemize}
\end{frame}

\begin{frame}{왜 통합 경고 컨셉이 필요했는가}
\begin{itemize}
  \item 스쿨존, 고속도로, 유도선 구간은 구간별 경고 규칙이 달라 별도 정책이 필요하다.
  \item 경찰차와 구급차 접근은 일반 구간 경고보다 우선해야 하며, 동시 접근 시에도 하나의 선택 규칙이 필요하다.
  \item 교차로 및 합류구간 위험은 긴급 이벤트와 객체 위험을 함께 고려해야 하므로 단일 기능 수준으로 설명하기 어렵다.
\end{itemize}
\end{frame}

\begin{frame}{우리가 정의한 핵심 컨셉}
\begin{itemize}
...
\begin{frame}{Test Oracle과 Test Matrix}
\begin{itemize}
...
\begin{frame}{CANoe-side companion docs의 의미}
\begin{itemize}
  \item \texttt{communication-matrix}, \texttt{owner-route}, \texttt{layer-separation-policy}, \texttt{oracle}, \texttt{acceptance-criteria}, \texttt{execution-guide} 같은 CANoe-side 문서를 별도로 유지하였다.
  \item 이 문서들은 구현 세부보다 경계, ownership, 판정 기준, 실행 규칙을 고정하는 역할을 맡았다.
  \item 결과적으로 제출용 00 문서와 CANoe-side 계약 문서가 서로 보완되는 구조가 만들어졌다.
\end{itemize}
\end{frame}

\section{시스템 구조}

\begin{frame}{전체 시스템 구조}
\begin{itemize}
...
\begin{frame}{채택 아키텍처: Option 1}
\begin{itemize}
  \item 채택 구조는 \texttt{Domain CAN + ETH\_Backbone + 중앙 경고 판정 체인}이다.
...
\begin{itemize}
  \item \textbf{Gateway / Backbone}: \texttt{CGW}, \texttt{SGW}, \texttt{DCM}, \texttt{IBOX}, \texttt{ETHB}
  \item \textbf{Domain Runtime Owner}: \texttt{VCU}, \texttt{IVI}, \texttt{ADAS}, \texttt{V2X}, \texttt{BCM}, \texttt{CLU}, \texttt{SCC}
  \item \textbf{Local Feature / Output Surface}: \texttt{ABS}, \texttt{EPB}, \texttt{AFLS}, \texttt{HUD}, \texttt{PGS}, \texttt{FCAM}, \texttt{SPAS}
  \item \textbf{Validation Harness}: \texttt{TEST\_SCN}, \texttt{TEST\_BAS}
\end{itemize}
\end{frame}

\begin{frame}{핵심 ECU 그룹}
\begin{itemize}
...
\begin{frame}{주요 ECU 역할 한눈에 보기}
\begin{itemize}
  \item \texttt{CGW}는 차량 내부 상태를 모으고 판단 계층으로 넘기는 입력 관문 역할을 맡는다.
  \item \texttt{V2X}는 외부 긴급 이벤트를 정규화하고, \texttt{ADAS}는 이를 객체 위험과 결합해 최종 경고를 선택한다.
  \item \texttt{BCM}, \texttt{CLU}, \texttt{HUD}, \texttt{AMP}는 선택된 경고를 조명, 계기, 전면 표시, 오디오 채널에 반영한다.
  \item \texttt{SGW}, \texttt{DCM}, \texttt{EXT\_DIAG}는 서비스 상태와 경계 조건을 독립적으로 관측하는 진단 계층으로 동작한다.
\end{itemize}
\end{frame}
...
\begin{frame}{왜 도메인 CAN + Backbone 구조인가}
\begin{itemize}
...
\begin{frame}{패널과 출력 채널}
\begin{itemize}
  \item \texttt{Ambient}, \texttt{Cluster}, \texttt{Navigation}, \texttt{Diagnostic Console} 패널은 프로젝트 결과를 가시적으로 확인하는 주요 표면이다.
  \item 패널은 단순 시각자료가 아니라, 출력 채널과 관측 채널이 실제로 연결되어 있음을 보여주는 운영 표면이다.
  \item 따라서 패널 구성은 설계와 구현, 시험을 연결하는 증거 중 하나로 사용되었다.
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item 따라서 운전자는 다수의 경고를 동시에 보는 대신, 현재 상황에서 가장 중요한 경고를 일관되게 인지할 수 있다.
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item 동시에 정상 경로가 복구되면 최소 경고를 유지한 채 안전하게 정상 상태로 복귀해야 한다.
  \item 따라서 본 컨셉에서 복귀 규칙은 경고 발생 규칙만큼 중요한 설계 요소로 취급되었다.
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item CAN ID는 \texttt{29-bit 정책}과 \texttt{11-bit active execution}을 분리해 문서와 실행 기준이 서로 충돌하지 않도록 정리하였다.
...
\begin{frame}{패널 구성 경험}
\begin{itemize}
  \item 패널은 단순 시연 화면이 아니라, 시스템 출력과 시험 관측을 함께 보여주는 운영 표면으로 활용되었다.
  \item Ambient, Cluster, Navigation, Diagnostic Console은 경고 채널과 상태 경계를 시각적으로 확인하는 주요 수단이었다.
  \item 패널 변경보다 CAPL과 sysvar 계약을 맞추는 방식으로 적응한 것이 중요한 경험이었다.
\end{itemize}
\end{frame}

\begin{frame}{Diagnostic Console의 의미}
\begin{itemize}
  \item Diagnostic Console은 서비스 상태, 경계 조건, 진단 상태를 독립적으로 확인하는 관측면으로 활용되었다.
  \item 이를 통해 경고 판단 문제와 서비스/경계 문제를 분리해 해석할 수 있었다.
  \item 즉 Diagnostic Console은 기능 화면이 아니라, 아키텍처 해석을 돕는 운영 도구에 가깝다.
\end{itemize}
\end{frame}

\begin{frame}{실제로 구현하면서 배운 점}
\begin{itemize}
...
\begin{itemize}
  \item 본 프로젝트는 문서만 정리한 것이 아니라, CANoe 환경에서 실제로 실행되고 관측 가능한 구조를 만들었다.
  \item CANoe Test Unit 실행 화면은 대표 시나리오가 시험 자산으로 연결되어 있음을 보여주고, 패널 및 Diagnostic Console은 출력과 상태 관측이 동시에 가능함을 보여준다.
  \item 따라서 실행 화면은 단순 시연 자료가 아니라, 설계와 시험 구조가 실제로 연결되었음을 보여주는 운영 증거이다.
\end{itemize}
\end{frame}

\begin{frame}{시험 자산을 어떻게 관리했는가}
\begin{itemize}
  \item 요구사항, HARA, 기능 정의를 기준으로 검증 계약, Test Oracle, Test Matrix, 단위시험·통합시험·시스템시험 자산을 구성하였다.
  \item 그 결과 시험 항목 번호가 아니라, 어떤 기능을 어떤 기준으로 검증하는지가 먼저 보이도록 정리할 수 있었다.
  \item 이 구조는 문서와 실행 자산을 같이 유지하는 데 핵심 역할을 했다.
\end{itemize}
\end{frame}

\begin{frame}{운영 자동화 경험}
\begin{itemize}
  \item 프로젝트는 구현과 시험에 그치지 않고, 결과 확인과 문서 closeout을 지원하는 운영 콘솔 구조까지 함께 구축하였다.
  \item 이를 통해 경고 컨셉, 시험 자산, 문서 산출, appendix bundle, 엑셀 산출이 하나의 운영 흐름으로 연결되었다.
  \item 즉 운영 자동화는 결과를 꾸미는 단계가 아니라, 기준선을 유지하는 실무 구조였다.
\end{itemize}
\end{frame}

\begin{frame}{별첨과 산출물 구성}
\begin{itemize}
  \item 소논문 본문 외에도 supplementary appendix bundle을 통해 안전, 계약, 검증, 진단 문서를 별도 PDF로 구조화하였다.
  \item 엑셀 산출은 00--07 문서를 기준으로 one-book 형태로 재생성할 수 있게 유지하였다.
  \item 이 산출물 구조는 발표 이후 심사 대응이나 부록 제출에 직접 활용할 수 있는 기준선이 되었다.
\end{itemize}
\end{frame}

\begin{frame}{산출물 패키지와 운영 워크스페이스}
\begin{itemize}
  \item 본 프로젝트의 실제 결과는 코드 하나가 아니라 문서, 시험 자산, 운영 콘솔, 엑셀 산출, 별첨 PDF까지 묶인 패키지이다.
  \item 이 패키지는 설계 설명, 시험 수행, 결과 확인, 제출 대응을 분리하지 않고 같은 기준선 위에서 운영할 수 있게 해준다.
  \item 따라서 발표에서는 기능 구현뿐 아니라, 이러한 운영 가능한 산출물 구조 자체를 주요 성과로 제시할 수 있다.
\end{itemize}
\end{frame}

\section{결과와 인사이트}

\begin{frame}{현재까지 도출된 결과}
\begin{itemize}
  \item 주행 경고 컨셉은 문서, CAPL ECU, 시험 자산, 운영 자동화까지 연결된 하나의 기준선으로 정리되었다.
  \item V2X 선택 규칙, ADAS 위험도 산정, timeout clear와 복귀 규칙이 각각 독립된 설명이 아니라 같은 중재 구조 안에서 설명되도록 정리되었다.
  \item 즉 본 프로젝트는 기능 데모보다 ``설명 가능한 컨셉 + 실행 가능한 시험 구조''를 구축한 데 의미가 있다.
\end{itemize}
\end{frame}
...
\begin{frame}{논문과 발표에서 강조할 결과}
\begin{itemize}
  \item 자산 수 자체보다, 어떤 로직을 어떤 지표로 해석할 수 있는 구조를 만들었는지가 더 중요하다.
  \item 따라서 결과는 단순 PASS 개수보다 V2X 선택, 통합 경고, 복귀 안정성의 세 축을 중심으로 정리한다.
  \item 필요 시 실제 CANoe 실행 화면과 대표 시나리오 결과를 함께 제시해 정량 지표의 의미를 보강할 수 있다.
\end{itemize}
\end{frame}

\begin{frame}{우리가 실제로 배운 점}
\begin{itemize}
  \item 핵심은 기능을 많이 추가하는 것이 아니라, 서로 다른 입력을 하나의 경고 정책으로 정리하는 구조를 세우는 일이었다.
  \item 설계, 구현, 시험, 운영 자동화는 따로 최적화할 수 없고 같은 추적 구조 안에서 함께 움직여야 했다.
  \item 혼합 CAN/Ethernet 경로에서는 기능 경로와 관측 경로를 함께 설계해야 실제 디버깅과 검증이 가능했다.
\end{itemize}
\end{frame}

\begin{frame}{프로젝트의 핵심 가치}
\begin{itemize}
  \item 구간 경고, 긴급차량 접근, 교차로·합류구간 위험을 분리된 기능이 아니라 하나의 경고 정책으로 설명할 수 있게 되었다.
  \item 문서, 코드, 시험, 운영 산출이 같은 기준선 위에 놓이도록 정리함으로써 이후 확장과 검증의 출발점을 확보하였다.
  \item 이 점이 본 프로젝트를 단순 시연이 아니라 시스템 관점의 구축 사례로 만든다.
\end{itemize}
\end{frame}
...
\begin{frame}{다음 단계}
\begin{itemize}
  \item 시험 기준 안정화와 정량 결과 확정
  \item XML 기반 결과 집계와 운영 자동화 고도화
...
\begin{frame}{맺음말}
\begin{itemize}
  \item 본 프로젝트는 주행 경고 컨셉을 설계, 구현, 시험, 운영 기준선으로 연결한 경험을 정리한 사례이다.
  \item 발표의 핵심은 ``어떤 기능을 만들었는가''보다 ``어떻게 설명 가능한 구조로 만들었는가''에 있다.
  \item 이후 단계에서는 현재 확보한 기준선을 바탕으로 정량 결과와 확장 기능을 점진적으로 누적할 수 있다.
\end{itemize}
\end{frame}
