...
\begin{abstract}
본 연구는 구간 정보, 긴급차량 접근 정보, 객체 위험 정보가 동시에 유입될 때 경고 우선순위와 복귀 규칙을 일관되게 처리하기 위한 실시간 주행 경고 컨셉을 대상으로 한다. 개별 경고 기능을 분리해 운용할 경우 우선순위 충돌, 표시 중복, timeout 이후 복귀 불안정이 반복되어 이를 하나의 계층형 경고 중재 구조로 통합할 필요가 있었다. 이를 위해 현대 차량 시스템을 참고한 기본 ECU 구성과 핵심 서비스 경로를 CANoe SIL 환경에 재구성하고, 라이선스 및 검증 범위를 고려하여 CAN과 Ethernet만을 공식 통신 범위로 사용하였다. 구현은 CAPL 기반 논리 ECU와 검증 하네스를 중심으로 수행하였으며, V2X 기반 긴급 이벤트 정규화, ADAS 기반 위험도 산정, 경고 중재 및 복귀 규칙을 실행 가능한 로직으로 구성하였다. 또한 요구사항, 기능, 통신, 변수, 코드, UT/IT/ST를 연결하는 추적 구조와 HARA, Master Test Matrix를 함께 유지하였다. 그 결과 본 연구는 구간 경고, 긴급 경고, 객체 위험 경고를 하나의 중재 구조 안에서 설명하고 반복 시험할 수 있는 기준선을 마련하였으며, 경고 우선순위, fail-safe, 복귀, 혼합 네트워크 경로를 동일한 구조 안에서 다룰 수 있음을 확인하였다.
\end{abstract}
...
\begin{editmemo}{그림 1 삽입 지시}
제목: V2X 긴급 이벤트 선택 및 timeout clear 상태도

구성: 다중 긴급 이벤트 입력, 차량 유형 우선순위 적용, ETA 비교, SourceID 동률 해소, 활성 이벤트 유지, watchdog timeout clear, 정상 상태 복귀

표현: 좌측에 ``EmergencyAlert 입력 집합'', 중앙에 ``V2X 정규화'', 우측에 ``활성 긴급 이벤트 $e_t^*$''

핵심 화살표: ``다중 입력 $\rightarrow$ 우선순위 선택 $\rightarrow$ 활성 이벤트 유지 $\rightarrow$ 무갱신 시 clear $\rightarrow$ 다음 입력 수신 시 재활성''
\end{editmemo}
...
\begin{editmemo}{그림 2 삽입 지시}
제목: ADAS 위험도 산정 및 계층형 경고 중재 블록도

구성: 차량 상태 $V_t$, 구간 맥락 $C_t$, 긴급 접근 정보 $E_t$, 객체 위험 정보 $O_t$, 서비스 가용성 $S_t$, $R_{emg}$, $TTC$, $R_{obj}$, $R_t$, hysteresis, 최종 경고 상태 $W_t$

표현: 입력 집합에서 위험도 계산 블록으로 수렴한 뒤, ``max($R_{emg}$, $R_{obj}$)''와 hysteresis를 거쳐 ``경고 레벨/유형'', ``감속 보조'', ``fail-safe 보정''으로 분기

핵심 화살표: ``긴급 접근/객체 위험 $\rightarrow$ 위험도 계산 $\rightarrow$ 계층형 중재 $\rightarrow$ HMI/감속 보조 출력''
\end{editmemo}
...
\begin{editmemo}{그림 3 삽입 지시}
제목: CAN/Ethernet 혼합 경고 전달 구조도

구성: CAN 기반 상태 수집 경로, Ethernet 기반 긴급 이벤트 유입 경로, V2X 정규화, ADAS 경고 중재, 출력 분배 경로, 관측 경로

표현: 하단 CAN 입력 버스와 상단 Ethernet 입력 버스가 중앙 ``V2X/ADAS'' 블록으로 모이고, 우측으로 ``BCM'', ``IVI'', ``CLU'', ``HUD'', ``AMP'', ``Decel Assist''가 분기되는 형태

핵심 화살표: ``CAN 상태 수집 $\rightarrow$ ADAS'', ``Ethernet 긴급 이벤트 $\rightarrow$ V2X $\rightarrow$ ADAS'', ``ADAS $\rightarrow$ HMI/감속 보조'', ``관측 경로는 별도 병렬 표기''
\end{editmemo}
...
구현은 CANoe SIL 환경에서 CAPL 기반 논리 ECU로 구성하였다~\cite{canoe}. 동력 계층은 EMS, TCU, VCU를 중심으로 차량 기초 거동을 생성하고, 차체 계층은 ESC와 MDPS를 중심으로 제동 및 조향 상태를 제공한다. 그 위에서 V2X는 긴급 이벤트 정규화와 활성 이벤트 유지, ADAS는 위험도 산정과 최종 경고 중재를 담당하며, BCM, IVI, CLU, HUD, AMP는 최종 경고를 시각 및 청각 표면으로 변환한다. SGW, DCM, IBOX, EXT\_DIAG는 서비스 상태와 진단 관측 경로를 담당한다.
...
\begin{editmemo}{그림 4 삽입 지시}
제목: CANoe SIL 구현 및 시험 구성도

구성: CAPL 논리 ECU군, Validation Harness, SysVar, 시험 시나리오 입력, UT/IT/ST 결과 연결

표현: 좌측 ``시험 입력/시나리오'', 중앙 ``CANoe SIL ECU 실행'', 우측 ``판정/로그/시험 결과'' 구조

핵심 화살표: ``시나리오 입력 $\rightarrow$ CAPL ECU 실행 $\rightarrow$ 상태 변수/출력 $\rightarrow$ 시험 판정''
\end{editmemo}
...
추적성은 ``Req $\rightarrow$ Func $\rightarrow$ Flow $\rightarrow$ Comm $\rightarrow$ Var $\rightarrow$ Code $\rightarrow$ UT/IT/ST''의 1:1 연결을 원칙으로 삼았다. 이를 위해 요구사항, 기능 정의, 네트워크 흐름, 통신 명세, 시스템 변수, 구현, 단위시험, 통합시험, 시스템시험 문서를 순차적으로 연결하고, 추가로 \texttt{00d\_HARA\_Worksheet}와 \texttt{00g\_Master\_Test\_Matrix}를 통해 상위 위험 항목과 시험 기준을 함께 관리하였다.
...
\begin{editmemo}{표 2 삽입 지시}
제목: V2X-ADAS 추적 구조 요약

권장 열: 상위 요구 / 핵심 기능 / 주요 변수 / 주요 CAPL 로직 / 대표 UT / 대표 IT / 대표 ST

권장 행:
\begin{enumerate}[label=\arabic*.]
\item 긴급 이벤트 정규화 / ETA-SourceID 선택 / emergencyType, EtaSeconds, SourceId / V2X / UT\_CORE\_004 / IT\_V2\_006 / ST\_V2\_016, ST\_V2\_017
\item TTC 기반 객체 위험 / objectTtcMin, objectRiskClass / ADAS / UT\_EXT\_006 / IT\_EXT\_018 / ST\_EXT\_027$\sim$029
\item 감속 보조 및 복귀 / decelAssistReq, timeoutClear / ADAS-CGW / UT\_CORE\_005 / IT\_V2\_010, IT\_V2\_011 / ST\_EXT\_025, ST\_EXT\_026
\end{enumerate}
\end{editmemo}
...
구현 단계에서는 CAPL 기반 ECU 로직과 검증 하네스를 통해 반복 실행 가능한 자산을 구성하였다. 특히 V2X는 긴급차량 유형, ETA, SourceID를 기준으로 활성 이벤트를 정규화하고, ADAS는 긴급 접근 위험도, 객체 기반 TTC 위험도, 감속 보조 임계값, stale와 fail-safe 보정을 포함한 단일 경고 상태를 산출하도록 구현하였다. 시험 단계에서는 UT, IT, ST 문서를 실행 가능한 행 중심으로 정리하고, Master Test Matrix를 통해 HARA와 시험 기준을 상위에서 연결하였다. 그 결과 본 프로젝트는 문서, 코드, 시험 자산이 함께 연동되는 경고 컨셉 시험 기반을 갖추게 되었다.
...
\begin{editmemo}{표 3 삽입 지시}
제목: 대표 시험 시나리오 요약

권장 열: 시험 레벨 / 시험 ID / 핵심 조건 / 확인 대상 / 기대 결과

권장 행:
\begin{enumerate}[label=\arabic*.]
\item IT / IT\_006 / 경찰-구급 동시 접근 / ETA, SourceID 규칙 / 단일 긴급 경고 일관 결정
\item IT / IT\_009 / 1000ms 무갱신 / timeout clear / 안전 해제 후 복귀
\item IT / IT\_018 / 긴급 접근 + TTC 위험 동시 발생 / 계층형 중재 / 긴급 경고 우선 + 감속 보조 판정
\item ST / ST\_018, ST\_019 / 외부 송신 주기 / Ethernet 경로 유지 / 100ms 주기 유지
\item ST / ST\_045, ST\_046 / fail-safe 및 복귀 / 최소 경고 유지와 정상 복귀 / 연속 시나리오 안정성
\end{enumerate}
\end{editmemo}
...
실제로 HARA는 ``긴급 신호 무갱신 시 경고를 안전하게 해제하고 정상 상태로 복귀해야 한다'', ``경고 정보 전달이 유지되어야 하며, 단절 시 강등 동작이 즉시 적용되어야 한다''고 정리하고 있으며~\cite{hara}, Master Test Matrix 역시 ST\_045, ST\_046을 통해 연속 주행과 fail-safe 복귀를 별도 항목으로 유지한다~\cite{master_test_matrix}. 이는 현재의 남은 과제가 단순 기능 추가보다 경계, 복귀, 전달 정책의 정합성에 더 가깝다는 점을 보여준다.
...
현대모비스 PBL 팀, ``00g\_Master\_Test\_Matrix.md,'' 버전 0.2, 2026-03-15.
