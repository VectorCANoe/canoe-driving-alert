\documentclass[8pt,a4paper]{article}
\usepackage{kotex}
\setmainhangulfont{AppleMyungjo}
\setsanshangulfont{Apple SD Gothic Neo}
\setmonofont{Apple SD Gothic Neo}
\usepackage[margin=12mm]{geometry}
\usepackage{longtable,booktabs,array,tabularx}
\usepackage{hyperref}
\usepackage{xurl}
\usepackage{graphicx}
\usepackage{adjustbox}
\usepackage{enumitem}
\usepackage{fancyhdr}
\usepackage{setspace}
\usepackage{parskip}
\usepackage{etoolbox}
\setlength{\parindent}{0pt}
\setlength{\parskip}{0.45em}
\setlength{\headheight}{14pt}
\setlength{\LTleft}{0pt}
\setlength{\LTright}{0pt}
\setlength{\tabcolsep}{1.8pt}
\setlist[itemize]{leftmargin=1.5em}
\setlist[enumerate]{leftmargin=1.8em}
\newcounter{none}
\providecommand{\tightlist}{%
  \setlength{\itemsep}{0pt}\setlength{\parskip}{0pt}}
\sloppy
\renewcommand{\arraystretch}{0.82}
\AtBeginEnvironment{longtable}{\tiny}
\AtBeginEnvironment{tabular}{\tiny}
\AtBeginEnvironment{tabularx}{\tiny}
\BeforeBeginEnvironment{tabular}{\begin{adjustbox}{max width=\textwidth}}
\AfterEndEnvironment{tabular}{\end{adjustbox}}
\BeforeBeginEnvironment{tabularx}{\begin{adjustbox}{max width=\textwidth}}
\AfterEndEnvironment{tabularx}{\end{adjustbox}}

\hypersetup{
  colorlinks=true,
  linkcolor=black,
  urlcolor=blue,
  pdftitle={주행 상황 실시간 경고 시스템 별첨 문서집},
  pdfauthor={Mobis PBL Team}
}

\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{별첨 문서집}
\fancyhead[R]{Driving Alert Project}
\fancyfoot[C]{\thepage}

\begin{document}

\begin{titlepage}
\centering
{\Large 주행 상황 실시간 경고 시스템 \par}
\vspace{0.5em}
{\LARGE 별첨 문서집 \par}
\vspace{0.8em}
{\large Supplementary Appendix Bundle \par}
\vspace{1.6em}
{\normalsize 본 문서집은 소논문 본문을 보강하기 위한 안전, 계약, 검증, 진단 문서를 묶어 정리한 reviewer-facing 별첨 자료이다. \par}
\vspace{1.2em}
\begin{tabular}{ll}
Project & Driving Alert Project \\
Bundle Scope & Governance / Runtime Contracts / Verification / Diagnostic \\
Language & Korean reviewer-facing edition \\
Source Rule & Canonical source paths are preserved in each supplementary note \\
\end{tabular}
\vfill
{\normalsize Workspace: \texttt{driving-alert-workproducts/governance/short-paper/appendix} \par}
\vspace{0.4em}
{\normalsize Generated from local source copies under \texttt{appendix/source/} \par}
\end{titlepage}

\tableofcontents
\clearpage

\section*{S1. 안전 및 추적성 기준}
\addcontentsline{toc}{section}{S1. 안전 및 추적성 기준}
본 별첨은 안전 목표, 위험 사건, ECU naming, CAN ID allocation, Test Matrix를 함께 제시하여 프로젝트의 안전성과 검증 추적성을 보강한다.

\subsection*{원문 기준 경로}
\begin{itemize}
\item \path{appendix/source/governance/project_v_model_traceability_map.md}
\item \path{appendix/source/governance/project_v_model_traceability_map.puml}
\item \texttt{driving-alert-workproducts/governance/00d\_HARA\_Worksheet.md}
\item \texttt{driving-alert-workproducts/governance/00e\_ECU\_Naming\_Standard.md}
\item \texttt{driving-alert-workproducts/governance/00f\_CAN\_ID\_Allocation\_Standard.md}
\item \texttt{driving-alert-workproducts/governance/00g\_Master\_Test\_Matrix.md}
\end{itemize}

\subsection*{프로젝트 V-모델과 추적 구조}
본 별첨의 V-모델 도식은 이론적 일반 모델이 아니라, 현재 active baseline에서 실제로 운영 중인 문서 체계와 검증 체계를 기준으로 정리하였다. 특히 \texttt{00d\_HARA\_Worksheet}, \texttt{01\_Requirements}, \texttt{03 / 0301 / 0302}, \texttt{0303 / 0304}, \texttt{04\_SW\_Implementation}, \texttt{05 / 06 / 07}의 연결 관계와 \texttt{00g\_Master\_Test\_Matrix}의 traceability 역할을 한 장에서 읽을 수 있도록 구성하였다.

\begin{figure}[h]
\centering
\includegraphics[width=0.80\textwidth]{../figures/project_v_model_traceability_map.png}
\caption{Project V-model and traceability map. Development-side documents, verification-side documents, and cross-cutting governance documents are arranged according to the active baseline used in this project.}
\end{figure}
\clearpage

\input{partials/project_v_model_traceability_map_appendix.tex}
\clearpage

\input{generated/00d_HARA_Worksheet.tex}
\clearpage
\input{generated/00e_ECU_Naming_Standard.tex}
\clearpage
\input{generated/00f_CAN_ID_Allocation_Standard.tex}
\clearpage
\input{generated/00g_Master_Test_Matrix.tex}
\clearpage

\section*{S2. 런타임 계약 세트}
\addcontentsline{toc}{section}{S2. 런타임 계약 세트}
본 별첨은 V2X, ADAS, CGW, 출력 계층의 ownership 경계와 CAN/Ethernet runtime contract를 보강한다.

\subsection*{아키텍처 읽기 레이어}
\begin{itemize}
\item \texttt{appendix/source/architecture/vehicle\_ecu\_architecture\_and\_interaction\_reference.md}
\item \texttt{canoe/docs/architecture/master\_book/ECU\_METADATA\_BOOK\_2026-03-28.pdf}
\item \texttt{canoe/docs/architecture/master\_book/ACTION\_FLOW\_INDEX\_2026-03-28.md}
\item \texttt{canoe/docs/architecture/master\_book/ECU\_ACTION\_FLOW\_MATRIX\_2026-03-28.md}
\item \texttt{canoe/docs/architecture/master\_book/ECU\_CARD\_INDEX\_2026-03-28.md}
\item \texttt{canoe/docs/architecture/master\_book/SIGNAL\_FLOW\_INDEX\_2026-03-28.md}
\end{itemize}

\input{generated/vehicle_ecu_architecture_and_interaction_reference.tex}
\clearpage

\subsection*{원문 기준 경로}
\begin{itemize}
\item \texttt{appendix/source/architecture/vehicle\_ecu\_architecture\_and\_interaction\_reference.md}
\item \texttt{canoe/docs/Kor/contracts/communication-matrix.md}
\item \texttt{canoe/docs/Kor/contracts/owner-route.md}
\item \texttt{canoe/docs/Kor/contracts/layer-separation-policy.md}
\item \texttt{canoe/docs/Kor/contracts/ethernet-interface.md}
\item \texttt{canoe/docs/Kor/contracts/multibus-policy.md}
\item \texttt{canoe/docs/Kor/contracts/panel-sysvar-contract.md}
\item \texttt{canoe/docs/Kor/contracts/ethernet-backbone.md}
\item \path{appendix/source/contracts/network_architecture_driving_alert_active_baseline.puml}
\item \path{appendix/source/contracts/signal_flow_integrated_warning_network.puml}
\end{itemize}

\subsection*{네트워크 설계도}
본 별첨의 네트워크 설계도는 현재 \texttt{canoe} 실행 구조를 기준으로 재작성하였다. 문서 최신화가 완전히 끝나지 않은 항목은 \texttt{V2X}, \texttt{ADAS}, \texttt{CGW}, \texttt{TEST\_SCN}, \texttt{TEST\_BAS}의 CAPL 구현과 runtime contract 문서를 함께 대조하여 owner, bus, observer 경계를 확정하였다.

\begin{figure}[h]
\centering
\includegraphics[width=0.92\textwidth]{../figures/network_architecture_driving_alert_active_baseline.png}
\caption{Integrated warning network topology. Domain CAN, ETH Backbone, decision ownership, boundary authority, render/output, validation/diagnostic separation을 한 장에서 정리하였다.}
\end{figure}
\clearpage

\begin{figure}[h]
\centering
\includegraphics[width=0.92\textwidth]{../figures/signal_flow_integrated_warning_network.png}
\caption{Integrated warning signal flow. Context publication, emergency ingress, V2X normalization, ADAS arbitration, CGW boundary shaping, render/verification, timeout recovery 규칙을 순차적으로 정리하였다.}
\end{figure}
\clearpage

\input{generated/communication-matrix.tex}
\clearpage
\input{generated/owner-route.tex}
\clearpage
\input{generated/layer-separation-policy.tex}
\clearpage
\input{generated/ethernet-interface.tex}
\clearpage
\input{generated/multibus-policy.tex}
\clearpage
\input{generated/panel-sysvar-contract.tex}
\clearpage
\input{generated/ethernet-backbone.tex}
\clearpage

\section*{S3. 검증 계약 세트}
\addcontentsline{toc}{section}{S3. 검증 계약 세트}
본 별첨은 시험 판정 기준, acceptance criteria, executable asset mapping, execution guide를 제시하여 논문의 검증 구조를 보강한다.

\subsection*{원문 기준 경로}
\begin{itemize}
\item \texttt{canoe/docs/Kor/verification/oracle.md}
\item \texttt{canoe/docs/Kor/verification/acceptance-criteria.md}
\item \texttt{canoe/docs/Kor/verification/test-asset-mapping.md}
\item \texttt{canoe/docs/Kor/verification/execution-guide.md}
\item \texttt{canoe/docs/Kor/verification/evidence-policy.md}
\end{itemize}

\input{generated/oracle.tex}
\clearpage
\input{generated/acceptance-criteria.tex}
\clearpage
\input{generated/test-asset-mapping.tex}
\clearpage
\input{generated/execution-guide.tex}
\clearpage
\input{generated/evidence-policy.tex}
\clearpage

\section*{S4. 진단 및 관측성 세트}
\addcontentsline{toc}{section}{S4. 진단 및 관측성 세트}
본 별첨은 서비스 상태와 경계 조건을 독립적으로 관측하는 진단·관측 구조를 보강한다.

\subsection*{원문 기준 경로}
\begin{itemize}
\item \texttt{canoe/docs/Kor/contracts/diagnostic-matrix.md}
\item \texttt{canoe/docs/Kor/contracts/diagnostic-sysvar-contract.md}
\item \texttt{canoe/docs/Kor/verification/diagnostic-seam-design.md}
\end{itemize}

\input{generated/diagnostic-matrix.tex}
\clearpage
\input{generated/diagnostic-sysvar-contract.tex}
\clearpage
\input{generated/diagnostic-seam-design.tex}

\end{document}
