...
\usepackage[top=20mm,bottom=23mm,left=18mm,right=18mm,columnsep=7mm]{geometry}
\usepackage{kotex}
...
\begin{figure*}[t]
\centering
\placeholderfigure{52mm}{다중 긴급 이벤트 입력, 차량 유형 우선순위 적용, ETA 비교, SourceID 동률 해소, 활성 이벤트 유지, watchdog timeout clear, 정상 상태 복귀를 포함한 상태도를 배치한다. 좌측은 EmergencyAlert 입력 집합, 중앙은 V2X 정규화, 우측은 활성 긴급 이벤트 $e_t^*$로 구성한다.}
...
\begin{figure*}[t]
\centering
\placeholderfigure{55mm}{차량 상태 $V_t$, 구간 맥락 $C_t$, 긴급 접근 정보 $E_t$, 객체 위험 정보 $O_t$, 서비스 가용성 $S_t$가 위험도 계산 블록으로 수렴하고, $R_{emg}$, $TTC$, $R_{obj}$, $R_t$ 및 hysteresis를 거쳐 최종 경고 상태 $W_t$, 감속 보조, fail-safe 보정으로 분기되는 블록도를 배치한다.}
...
\begin{figure*}[t]
\centering
\placeholderfigure{54mm}{CAN 기반 상태 수집 경로와 Ethernet 기반 긴급 이벤트 유입 경로가 중앙의 V2X/ADAS 블록으로 모이고, 우측으로 BCM, IVI, CLU, HUD, AMP, Decel Assist가 분기되는 혼합 네트워크 전달 구조도를 배치한다. 관측 경로는 운영 경로와 병렬로 별도 표기한다.}
...
\begin{figure*}[t]
\centering
\placeholderfigure{52mm}{좌측의 시험 입력 및 시나리오, 중앙의 CANoe SIL CAPL 논리 ECU군과 Validation Harness, 우측의 상태 변수·로그·UT/IT/ST 결과 판정으로 이어지는 구현 및 시험 구성도를 배치한다.}
...
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.2cm} >{\raggedright\arraybackslash}p{2.7cm} >{\raggedright\arraybackslash}p{3.2cm} >{\raggedright\arraybackslash}p{2.1cm} >{\raggedright\arraybackslash}p{1.9cm} >{\raggedright\arraybackslash}p{1.9cm} >{\raggedright\arraybackslash}X}
\toprule
상위 요구 & 핵심 기능 & 주요 변수 & 주요 CAPL 로직 & 대표 UT & 대표 IT & 대표 ST \\
\midrule
긴급 접근 경고 & 긴급 이벤트 정규화 및 ETA-SourceID 선택 & emergencyType,\newline EtaSeconds,\newline SourceId & V2X & UT\_CORE\_004 & IT\_V2\_006 & ST\_V2\_016,\newline ST\_V2\_017 \\
객체 위험 경고 & TTC 기반 객체 위험 산정 & objectTtcMin,\newline objectRiskClass & ADAS & UT\_EXT\_006 & IT\_EXT\_018 & ST\_EXT\_027$\sim$029 \\
감속 보조 및 복귀 & 감속 보조, timeout clear, 복귀 & decelAssistReq,\newline timeoutClear & ADAS-CGW & UT\_CORE\_005 & IT\_V2\_010,\newline IT\_V2\_011 & ST\_EXT\_025,\newline ST\_EXT\_026 \\
\bottomrule
\end{tabularx}
\end{table*}
...
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{1.7cm} >{\raggedright\arraybackslash}p{2.1cm} >{\raggedright\arraybackslash}p{3.6cm} >{\raggedright\arraybackslash}p{3.0cm} >{\raggedright\arraybackslash}X}
\toprule
...
IT & IT\_018 & 긴급 접근과 TTC 위험 동시 발생 & 계층형 중재 & 긴급 경고 우선 및 감속 보조 판정 \\
ST & ST\_018, ST\_019 & 외부 송신 주기 유지 & Ethernet 경로 유지 & 100ms 주기 유지 \\
