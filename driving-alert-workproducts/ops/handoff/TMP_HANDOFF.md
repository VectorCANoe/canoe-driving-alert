...
- Root Canonical Sync Target:
  - `driving-alert-workproducts/00/01/03/0301/0302/0303/0304/04/05/06/07`
...
- Do not do:
  - `driving-alert-workproducts/` 정본 SoT 직접 수정
...
- Do not do:
  ...
  - `driving-alert-workproducts/` 정본 SoT 직접 rewrite
...
- 문서팀은 `driving-alert-workproducts/` 정본/제출 문서 변경만 commit/push한다.
...
- 원격 변경이 `canoe/`만 포함하면 `canoe/`만 선택 반영하고 `driving-alert-workproducts/`는 유지한다.
- 원격 변경이 `driving-alert-workproducts/`까지 포함하면 문서 기준선 훼손 여부를 먼저 검토하고, 필요 시 문서팀 기준 백업 브랜치에서 복원한 뒤 후행 반영한다.
...
- 공식 레포 전환 준비 시 `.gitignore` 대상(`reference/`, `archive/`, local evidence/log/tmp 산출물)은 Git 이력/PR diff에서 직접 확인되지 않을 수 있다.
- 위 자산 검토가 필요하면 반드시 로컬 경로에서 직접 열어 확인한다.
