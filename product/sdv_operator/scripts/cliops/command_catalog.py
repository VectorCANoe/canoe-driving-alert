...
PRODUCT_COMMAND_GROUPS: dict[str, list[PaletteCommand]] = {
    "Primary Workflow": [
        ...
        PaletteCommand(
            ...
            use_when=(
                ...
                "운영 문서에 반영하기 전 증빙 준비 상태를 빠르게 점검할 때",
            ),
            success_signals=(
                "run_readiness overall_status가 PASS 또는 준비 가능한 상태로 생성됨",
                "증빙 폴더와 readiness json/md가 생성됨",
            ),
            expected_outputs=(
    ...
    "Runtime Support": [
        ...
        PaletteCommand(
            ...
            windows_only=True,
            notes="CI/CD와 Jenkins 연동에 사용하는 표준 배치 경로입니다. CANoe 결과와 정규화된 배치 산출물을 함께 남깁니다.",
            use_when=(
                "CI/CD 또는 야간 회귀에서 기계 판독형 결과를 남겨야 할 때",
            ...
            next_step="Results와 Artifacts에서 배치 결과와 생성 산출물을 확인한 뒤, Jenkins 수집 경로까지 점검하십시오.",
            params=(
                CommandParam(
                    key="run_id",
                    flag="--run-id",
                ...
                CommandParam(
                    ...
                    label="Campaign ID",
                    default="{campaign_id}",
                    help="반복 실행과 CI/CD 묶음을 식별하는 campaign 키입니다.",
                    placeholder="CMP_20260310",
                    required=True,
                ),
                CommandParam(
        ...
        PaletteCommand(
            ...
            summary="증빙, marker, score 준비 상태를 점검합니다.",
            use_when=(
        ...
        PaletteCommand(
            ...
            summary="배치/doctor/readiness 결과를 BCM·IVI·CLU·ADAS·V2X 같은 surface ECU 기준으로 다시 묶습니다.",
            notes="결과와 archive 구조를 runtime module이 아니라 current official surface ECU 기준으로 정리할 때 사용합니다.",
            use_when=(
                "배치 결과를 surface ECU 기준으로 재정렬해야 할 때",
                "archive를 surface ECU 번들 단위로 나눠야 할 때",
            ),
            success_signals=(
            ...
            next_step="Results와 Artifacts에서 surface ECU 기준 결과를 확인하십시오.",
        ),
        PaletteCommand(
    ...
    "Packaging": [
        PaletteCommand(
            command_id="verify.bind_doc",
            title="문서 바인딩 묶음",
            command="verify bind-doc --run-id 20260308_0900",
            base_command="verify bind-doc",
            summary="05/06/07 문서 ID와 scored evidence를 READY / DOC_ONLY / EVIDENCE_ONLY로 묶습니다.",
            notes="closeout 직전 문서 팀에 넘길 binding matrix를 만드는 단계입니다.",
            use_when=(
                "실행 결과를 05/06/07 문서 ID 기준으로 다시 맞춰야 할 때",
                "문서에는 있는데 evidence가 없는 항목과, evidence는 있는데 문서 ID가 없는 항목을 나눠 볼 때",
            ),
            success_signals=(
                "doc_binding_bundle.csv/json/md 생성",
                "scenario_id / native_asset / expected / rule_type / rule_ms 가 carry-forward 됨",
            ),
            expected_outputs=(
            ...
            failure_focus=(
                "DOC_ONLY와 EVIDENCE_ONLY가 어떤 tier/test_id에 몰리는지 확인",
                "문서 ID와 native asset rename drift가 있는지 확인",
            ),
            next_step="문서 반영용 fill template이 필요하면 바로 verify fill-template로 이어가십시오.",
            params=(
                CommandParam(
                    key="run_id",
                    flag="--run-id",
                    label="Run ID",
                    default="{run_id}",
                    help="closeout 대상으로 묶을 실행 식별자입니다.",
                    placeholder="20260308_0900",
                    required=True,
                ),
            ),
        ...
        PaletteCommand(
            command_id="verify.fill_template",
            title="문서 반영 템플릿",
            command="verify fill-template --run-id 20260308_0900 --owner-fallback DEV2",
            base_command="verify fill-template",
            summary="05/06/07 문서 업데이트용 Pass/Fail, owner, date, evidence 링크 템플릿을 만듭니다.",
            notes="READY 행은 문서 반영, 미완성 행은 추가 액션이 무엇인지 바로 드러내는 closeout 템플릿입니다.",
            use_when=(
                "문서 팀이 그대로 채워 넣을 CSV/Markdown 템플릿이 필요할 때",
                "READY 행과 RUN_AND_SCORE_REQUIRED 행을 빠르게 나눠야 할 때",
            ),
            success_signals=(
                "doc_fill_template.csv/md 생성",
                "REVIEW_READY_ROW / RUN_AND_SCORE_REQUIRED / ADD_DOC_ID_OR_RENAME_TEST 액션이 계산됨",
            ),
            expected_outputs=(
            ...
            failure_focus=(
                "owner/date fallback이 비어 있지 않은지 확인",
                "READY인데 final verdict가 비어 REVIEW_READY_ROW로 남는 항목 확인",
            ),
            next_step="최종 closeout 묶음이 필요하면 verify finalize로 insight와 binding/template를 함께 생성하십시오.",
            params=(
                CommandParam(
                    key="run_id",
                    flag="--run-id",
                ...
                CommandParam(
                    ...
                    default="DEV2",
                    help="READY 행에서 owner가 비어 있을 때 채울 기본값입니다.",
                    placeholder="DEV2",
                    required=True,
                ),
            ),
        ...
        PaletteCommand(
            command_id="verify.finalize",
            title="검증 closeout",
            command="verify finalize --run-id 20260308_0900 --owner DEV2",
            base_command="verify finalize",
            summary="run insight, doc binding, doc fill template, readiness closeout 결과를 한 번에 생성합니다.",
            notes="최종 reviewer/Jenkins/archive closeout 전에 현재 run_id를 한 번 더 정리하는 표준 마감 단계입니다.",
            use_when=(
                "하나의 run_id를 closeout 가능한 상태로 정리할 때",
                "run insight와 문서 반영용 결과를 같은 시점 기준으로 다시 만들고 싶을 때",
            ),
            success_signals=(
                "run_insight_report + doc_binding_bundle + doc_fill_template 생성",
                "closeout carry-forward 필드가 같은 run 기준으로 정리됨",
            ),
            expected_outputs=(
            ...
            failure_focus=(
                "scored evidence 누락 tier",
                "strict metadata/axis 실패와 binding drift 원인",
            ),
            next_step="Results와 Artifacts에서 insight / binding / fill template을 확인한 뒤 archive/materialize로 이어가십시오.",
            params=(
                CommandParam(
                    key="run_id",
                    flag="--run-id",
                ...
                CommandParam(
                    ...
                    label="담당자",
                    default="DEV2",
                    help="finalize 보고서와 fill template fallback에 기록되는 운영자 태그입니다.",
                    placeholder="DEV2",
                    required=True,
                ),
            ),
        ...
        PaletteCommand(
            ...
            title="Source Contract 목록",
            command="artifact list --scope source",
            summary="산출물 생성에 쓰이는 Source Contract, official 05/06/07, CANoe verification 기준 문서를 확인합니다.",
            notes="surface inventory, traceability profile, 05/06/07, test-asset-mapping, execution-guide, closeout/evidence policy를 같이 보여줍니다.",
            use_when=(
        ...
        PaletteCommand(
            ...
            summary="surface ECU 기준 결과 문서를 바로 엽니다.",
            use_when=(
                "BCM/IVI/CLU/ADAS/V2X 기준 결과를 바로 확인할 때",
            ),
        ...
        PaletteCommand(
            ...
            next_step="surface bundle이나 실행 매니페스트까지 이어서 확인하십시오.",
        ),
        PaletteCommand(
            command_id="artifact.open_run_insight",
            title="Run Insight 열기",
            command="artifact open --target run-insight",
            summary="현재 run의 tier coverage, hotspot, timing insight 요약을 바로 엽니다.",
            use_when=(
                "closeout 전에 어떤 tier/hotspot이 병목인지 먼저 읽고 싶을 때",
            ),
            success_signals=("run_insight_report.md가 열림",),
            expected_outputs=("canoe/tmp/reports/verification/run_insight_report.md",),
            next_step="문서 반영 단계면 doc binding bundle과 fill template도 이어서 확인하십시오.",
        ),
        PaletteCommand(
            command_id="artifact.open_doc_binding_bundle",
            title="문서 바인딩 묶음 열기",
            command="artifact open --target doc-binding-bundle",
            summary="05/06/07 문서 ID와 scored evidence 매칭 상태를 Markdown으로 바로 엽니다.",
            use_when=(
                "READY / DOC_ONLY / EVIDENCE_ONLY 분포를 사람 읽기용 리포트로 확인할 때",
            ),
            success_signals=("doc_binding_bundle.md가 열림",),
            expected_outputs=("canoe/tmp/reports/verification/doc_binding_bundle.md",),
            next_step="문서 팀 반영 전에는 fill template도 함께 확인하십시오.",
        ),
        PaletteCommand(
            command_id="artifact.open_doc_fill_template",
            title="문서 반영 템플릿 열기",
            command="artifact open --target doc-fill-template",
            summary="05/06/07 문서 반영용 Pass/Fail, owner, date, evidence 링크 템플릿을 엽니다.",
            use_when=(
                "문서 업데이트 직전 어떤 행이 바로 반영 가능한지 확인할 때",
            ),
            success_signals=("doc_fill_template.md가 열림",),
            expected_outputs=("canoe/tmp/reports/verification/doc_fill_template.md",),
            next_step="READY 행과 action_required 값을 기준으로 문서 closeout 순서를 확정하십시오.",
        ),
        PaletteCommand(
            ...
            summary="가장 최근 archive run의 실행 매니페스트를 엽니다.",
            use_when=(
        ...
        PaletteCommand(
            ...
            use_when=(
                "BCM/IVI/CLU/ADAS/V2X 기준 증빙 폴더를 archive 기준으로 바로 보고 싶을 때",
            ),
            ...
            next_step="실행 매니페스트와 함께 어떤 run에서 나온 bundle인지 대조하십시오.",
        ),
        PaletteCommand(
        ...
        PaletteCommand(
            ...
            summary="운영 profile과 active baseline suite profile 원본 JSON을 바로 엽니다.",
            use_when=(
                "Automation/Campaign 실행 기본값이 어떤 profile에서 오는지 확인할 때",
                "CI/CD 또는 Jenkins가 어떤 반복/간격/지속시간 규칙을 쓰는지 확인할 때",
            ),
            success_signals=("campaign_profiles.json이 열림",),
            expected_outputs=("product/sdv_operator/config/campaign_profiles.json",),
            next_step="필요하면 역할 경계 문서와 함께 Jenkins/Console 경계를 확인하십시오.",
        ),
        PaletteCommand(
            command_id="artifact.open_unit_test_doc",
            title="원본 05 Unit Test 문서 열기",
            command="artifact open --target unit-test-doc",
            summary="공식 05 Unit Test 문서를 바로 엽니다.",
            use_when=(
                "closeout 전에 UT 문서 행을 직접 확인해야 할 때",
                "doc binding이나 fill template 결과를 05 문서 원본과 대조할 때",
            ),
            success_signals=("05_Unit_Test.md가 열림",),
            expected_outputs=("driving-alert-workproducts/05_Unit_Test.md",),
            next_step="test asset mapping이나 doc binding bundle과 함께 UT 행을 대조하십시오.",
        ),
        PaletteCommand(
            command_id="artifact.open_integration_test_doc",
            title="원본 06 Integration Test 문서 열기",
            command="artifact open --target integration-test-doc",
            summary="공식 06 Integration Test 문서를 바로 엽니다.",
            use_when=(
                "closeout 전에 IT 문서 행을 직접 확인해야 할 때",
                "doc binding이나 fill template 결과를 06 문서 원본과 대조할 때",
            ),
            success_signals=("06_Integration_Test.md가 열림",),
            expected_outputs=("driving-alert-workproducts/06_Integration_Test.md",),
            next_step="test asset mapping이나 doc binding bundle과 함께 IT 행을 대조하십시오.",
        ),
        PaletteCommand(
            command_id="artifact.open_system_test_doc",
            title="원본 07 System Test 문서 열기",
            command="artifact open --target system-test-doc",
            summary="공식 07 System Test 문서를 바로 엽니다.",
            use_when=(
                "closeout 전에 ST 문서 행을 직접 확인해야 할 때",
                "doc binding이나 fill template 결과를 07 문서 원본과 대조할 때",
            ),
            success_signals=("07_System_Test.md가 열림",),
            expected_outputs=("driving-alert-workproducts/07_System_Test.md",),
            next_step="test asset mapping이나 doc binding bundle과 함께 ST 행을 대조하십시오.",
        ),
        PaletteCommand(
            ...
            success_signals=("test-asset-mapping.md가 열림",),
            expected_outputs=("canoe/docs/verification/test-asset-mapping.md",),
            next_step="test units guide와 execution guide를 함께 열어 suite와 실행 기준을 대조하십시오.",
        ),
        PaletteCommand(
            ...
            success_signals=("test_units/README.md가 열림",),
            expected_outputs=("canoe/tests/modules/test_units/README.md",),
            next_step="필요하면 test asset mapping과 함께 05/06/07 매핑을 대조하십시오.",
        ),
        PaletteCommand(
        ...
        PaletteCommand(
            command_id="artifact.open_closeout_standard",
            title="원본 closeout standard 열기",
            command="artifact open --target closeout-standard",
            summary="UT/IT/ST official closeout과 FULL regression 경계를 정의한 기준 문서를 바로 엽니다.",
            use_when=(
                "FULL 결과가 official closeout을 대체하지 않는다는 기준을 다시 확인해야 할 때",
                "문서 PASS/FAIL 반영 전에 tier authority를 명확히 확인해야 할 때",
            ),
            success_signals=("VECTOR_ALIGNED_CLOSEOUT_STANDARD.md가 열림",),
            expected_outputs=("canoe/docs/verification/VECTOR_ALIGNED_CLOSEOUT_STANDARD.md",),
            next_step="execution guide와 evidence policy를 함께 열어 실행 authority와 evidence seed 기준을 대조하십시오.",
        ),
        PaletteCommand(
            command_id="artifact.open_evidence_policy",
            title="원본 evidence policy 열기",
            command="artifact open --target evidence-policy",
            summary="verification_log, native report, 05/06/07 seed 규칙을 정의한 증빙 정책 문서를 바로 엽니다.",
            use_when=(
                "doc binding이나 fill template 결과의 seed source가 무엇인지 다시 확인해야 할 때",
                "evidence root와 incoming raw_write_window 규칙을 바로 확인해야 할 때",
            ),
            success_signals=("evidence-policy.md가 열림",),
            expected_outputs=("canoe/docs/verification/evidence-policy.md",),
            next_step="test asset mapping과 05/06/07 문서를 함께 열어 1:1 evidence seed를 확인하십시오.",
        ),
        PaletteCommand(
            ...
            summary="UT/IT/ST official active baseline과 FULL regression baseline 매트릭스를 바로 엽니다.",
            use_when=(
        ...
        PaletteCommand(
            ...
            summary="surface bundle이 어떤 official TestCase/Scenario/native asset 매핑 기준을 쓰는지 원본 JSON을 엽니다.",
        ...
        PaletteCommand(
            ...
            summary="CANoe TEST가 하는 일, Jenkins가 하는 일, 이 콘솔이 하는 일을 한 문서에서 확인합니다.",
            use_when=(
            ...
            next_step="역할 경계, campaign profiles, CI/CD / Jenkins 문서를 같이 열어 경계를 확정하십시오.",
        ),
        PaletteCommand(
            ...
            use_when=("산출물 경로가 어디로 materialize되는지 기준 파일을 보고 확인할 때",),
        ...
        PaletteCommand(
            ...
            title="CI/CD / Jenkins 문서 열기",
            command="artifact open --target ci-bridge-doc",
            summary="CI/CD, Jenkins, Verification Console 사이의 연결 규약 문서를 엽니다.",
            use_when=(
            ...
            next_step="batch/JUnit/archive 설정은 Jenkins 샘플과 함께 확인하십시오.",
        ),
        PaletteCommand(
            ...
            next_step="CI/CD / Jenkins 문서와 함께 역할 경계를 확인하십시오.",
        ),
        PaletteCommand(
